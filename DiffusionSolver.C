...
DiffusionSolver_RegularQuad<hyperedge_dim>::DiffusionSolver_RegularQuad
...
*/  
  ...
  const unsigned int num_ansatz_fct = pow ( max_poly_degree_ + 1, hyperedge_dim );
  const unsigned int num_ansatz_bdr = pow ( max_poly_degree_ + 1, hyperedge_dim - 1 );
...
template<unsigned int hyperedge_dim>
unsigned int DiffusionSolver_RegularQuad<hyperedge_dim>::loc_matrix_index(const unsigned int row, const unsigned int column) const
{
  assert( 0 <= row && row < (hyperedge_dim + 1) * pow((max_poly_degree_ + 1), hyperedge_dim) );
  assert( 0 <= column && column < (hyperedge_dim + 1) * pow((max_poly_degree_ + 1), hyperedge_dim) );
  return column * (hyperedge_dim + 1) * pow((max_poly_degree_ + 1), hyperedge_dim) + row;  // Transposed for LAPACK
}


template<unsigned int hyperedge_dim>
vector<double> DiffusionSolver_RegularQuad<hyperedge_dim>::assemble_loc_mat() const
{
  double hyperedge_area = 1.;
  unsigned int num_ansatz_fct = pow ( max_poly_degree_ + 1, hyperedge_dim );
  unsigned int num_ansatz_bdr = pow ( max_poly_degree_ + 1, hyperedge_dim - 1 );
  vector<double> local_mat( (hyperedge_dim+1) * num_ansatz_fct * (hyperedge_dim+1) * num_ansatz_fct , 0.);
  ...
  for (unsigned int dim = 0; dim < hyperedge_dim; ++dim)
    for (unsigned int i = 0; i < num_ansatz_fct; ++i)
    {
      ...
      for (unsigned int j = 0; j < num_ansatz_fct; ++j)
        for (unsigned int q = 0; q < num_of_quad_; ++q)
          local_mat[loc_matrix_index(  dim * num_ansatz_fct + i , hyperedge_dim * num_ansatz_fct + j )] -=
  ...
  for (unsigned int i = 0; i < num_ansatz_fct; ++i)
  {
    for (unsigned int dim = 0; dim < hyperedge_dim; ++dim)
    {
      ...
      {
        for (unsigned int q = 0; q < num_of_quad_; ++q)
          local_mat[loc_matrix_index( hyperedge_dim * num_ansatz_fct + i , dim * num_ansatz_fct + j )] -=
            quad_weights_[q] * derivs_quad_[dim][i][q] * trials_quad_[j][q];
      ...
      for (unsigned int j = 0; j < num_ansatz_fct; ++j)
        for (unsigned int q = 0; q < num_quad_bdr_; ++q)
          local_mat[loc_matrix_index( hyperedge_dim * num_ansatz_fct + i , hyperedge_dim * num_ansatz_fct + j )] +=
...
/*  
  cout << endl << endl;
  for (unsigned int i = 0; i < (hyperedge_dim + 1) * pow((max_poly_degree_ + 1), hyperedge_dim); ++i)
  {
    for(unsigned int j = 0; j < (hyperedge_dim + 1) * pow((max_poly_degree_ + 1), hyperedge_dim); ++j)
...
template<unsigned int hyperedge_dim>
vector<double> DiffusionSolver_RegularQuad<hyperedge_dim>::assemble_rhs(const vector< vector<double> >& lambda_values) const
{
  unsigned int num_ansatz_fct = pow ( max_poly_degree_ + 1, hyperedge_dim );
  unsigned int num_ansatz_bdr = pow ( max_poly_degree_ + 1, hyperedge_dim - 1 );
  vector<double> right_hand_side((hyperedge_dim+1) * num_ansatz_fct, 0.);
  ...
  for (unsigned int ansatz = 0; ansatz < num_ansatz_fct; ++ansatz)
    for (unsigned int dim = 0; dim < hyperedge_dim; ++dim)
      for (unsigned int bdr_ans = 0; bdr_ans < num_ansatz_bdr; ++bdr_ans)
        for (unsigned int q = 0; q < num_quad_bdr_; ++q)
          right_hand_side[hyperedge_dim * num_ansatz_fct + ansatz] += tau_ * quad_bdr_[q] *
...
template<unsigned int hyperedge_dim>
vector<double> DiffusionSolver_RegularQuad<hyperedge_dim>::solve_local_system_of_eq(vector<double>& loc_matrix, vector<double>& loc_rhs) const
{
  assert( loc_matrix.size() == loc_rhs.size() * loc_rhs.size() );
  const int system_size = loc_rhs.size();
...
template<unsigned int hyperedge_dim>
vector<double> DiffusionSolver_RegularQuad<hyperedge_dim>::solve_local_problem(const vector< vector<double> >& lambda_values) const
{
...
template<unsigned int hyperedge_dim>
vector< vector<double> > DiffusionSolver_RegularQuad<hyperedge_dim>::dual_at_boundary(const vector<double>& coeffs) const
{
  unsigned int num_ansatz_fct = pow ( max_poly_degree_ + 1, hyperedge_dim );
  unsigned int num_ansatz_bdr = pow ( max_poly_degree_ + 1, hyperedge_dim - 1 );
  
  vector< vector<double> > bdr_values;
  bdr_values.resize(2 * hyperedge_dim);
...
template<unsigned int hyperedge_dim>
vector< vector<double> > DiffusionSolver_RegularQuad<hyperedge_dim>::primal_at_boundary(const vector<double>& coeffs) const
{
  unsigned int num_ansatz_fct = pow ( max_poly_degree_ + 1, hyperedge_dim );
  unsigned int num_ansatz_bdr = pow ( max_poly_degree_ + 1, hyperedge_dim - 1 );
  ...
  for (unsigned int dim = 0; dim < hyperedge_dim; ++dim)
  {
    ...
    for (unsigned int bdr_ansatz = 0; bdr_ansatz < num_ansatz_bdr; ++bdr_ansatz)
      for (unsigned int ansatz = 0; ansatz < num_ansatz_fct; ++ansatz)
        for (unsigned int q = 0; q < num_quad_bdr_; ++q)
        {
          bdr_values[2*dim+0][bdr_ansatz] += quad_bdr_[q] * coeffs[hyperedge_dim * num_ansatz_fct + ansatz] * trials_bound_[2*dim+0][ansatz][q] * bound_trials_quad_[bdr_ansatz][q];
          bdr_values[2*dim+1][bdr_ansatz] += quad_bdr_[q] * coeffs[hyperedge_dim * num_ansatz_fct + ansatz] * trials_bound_[2*dim+1][ansatz][q] * bound_trials_quad_[bdr_ansatz][q];
        }
  }
...
template<unsigned int hyperedge_dim>
vector< vector<double> > DiffusionSolver_RegularQuad<hyperedge_dim>::numerical_flux_at_boundary(const vector< vector<double> >& lambda_values, const vector<double>& coeffs) const
{
  vector< vector<double> > bdr_values;
...
template<unsigned int hyperedge_dim>
vector< vector<double> > DiffusionSolver_RegularQuad<hyperedge_dim>::numerical_flux_from_lambda(const vector< vector<double> >& lambda_values) const
{
