...
\begin{table}[t]
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  & $i = 2$ && \textcolor{red}{7.68e-2} & --- && \textcolor{red}{3.73e-4} & --- && \textcolor{red}{1.07e-6} & ---  \\
  & $i = 3$ && 3.65e-3 & 4.4 && 5.10e-6 & 6.2 && 3.86e-9 & 8.1  \\
  & $i = 4$ && 2.03e-4 & 4.2 && 7.49e-8 & 6.1 && 1.44e-11 & 8.1  \\
  & $i = 5$ && 1.20e-5 & 4.0 && 1.13e-9 & 6.0 && 5.50e-14 & 8.0  \\
  \midrule
  ...
  & $i = 5$ && 3.85e-4 & 3.1 && 3.63e-8 & 5.0 && 1.06e-11 & 4.4  \\
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 2$, SI}}
  & $i = 2$ && 1.92e-2 & --- && 9.33e-5 & --- && 2.69e-7 & ---  \\
  & $i = 3$ && 4.56e-4 & 5.4 && 6.38e-7 & 7.2 && 4.83e-10 & 9.1  \\
  & $i = 4$ && 1.27e-5 & 5.2 && 4.68e-9 & 7.1 && \textcolor{red}{4.86e-4} & 5  \\
  & $i = 5$ && 3.76e-7 & 5.1 && \textcolor{red}{2.12e-3} & 5 && 4.86e-4 & 5  \\
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 2$, NL}}
  & $i = 2$ && 5.68e-1 & --- && 2.98e-3 & --- && 8.61e-6 & ---  \\
  & $i = 3$ && 5.80e-2 & 3.3 && 8.16e-5 & 5.2 && \textcolor{red}{6.18e-8} & 7.1  \\
  & $i = 4$ && 6.50e-3 & 3.2 && \textcolor{red}{2.39e-6} & 5.1 && 4.86e-4 & 5  \\
  & $i = 5$ && 7.71e-5 & 3.1 && 2.12e-3 & 5 && 4.86e-4 & 5  \\
  \bottomrule
 \end{tabular}\vspace{1ex}
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc) of the smallest eigenvalue. All results use the accurate local problems for the shifted inverse (SI) and nonlinear (NL) approaches.}\label{TAB:eigenvalue_conv}
\end{table}
% 
For an numerical test, we try to approximate the smallest $\eigenval$ that suffices \eqref{EQ:diffusion_eigen} on the unit hypercube, i.e., $\eigenval = \globDim \pi^2$. As starting value for the nonlinear (NL) approach, we use the result of the shifted inverse (SI) approach, for which $\sigma = \eigenval / 2$ is chosen. The absolute distances / errors (err) of the eigenvalue and its approximate, and the deduced estimated orders of convergence can be found in, Table \ref{TAB:eigenvalue_conv}. Here, we observe that the order of convergence is $2p+1$ for the expensive nonlinear problem. This is the expected order of convergence. Again, we stress that the shifted inverse approach is solely to gain a good starting point for Newton's method. Its results do not necessarily approximate the desired values. To find a remedy for this issue, we consider the approximation to the statically condensed mass matrix of Section \ref{SEC:approx_mass}.
% 
...
\begin{table}
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  & $i = 2$ && \textcolor{red}{5.17e-1} & --- && \textcolor{red}{5.17e-1} & --- && \textcolor{red}{5.17e-1} & ---  \\
  ...
  \bottomrule
 \end{tabular}\vspace{1ex}
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc) for diffusive eigenvalue example. Here, the approximate mass matrix and a shifted inverse approach are used instead of the exact local problems.}\label{TAB:eigs_conv_m}
\end{table}
% 
...
\begin{table}[t]
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 1$, SI}}
  & $i = 2$ && \textcolor{red}{1.51e-2} & --- && \textcolor{red}{7.49e-5} & --- && \textcolor{red}{2.16e-7} & ---  \\
  & $i = 3$ && 7.31e-4 & 4.4 && 1.02e-6 & 6.2 && 7.75e-10 & 8.1  \\
  & $i = 4$ && 4.08e-5 & 4.2 && 1.50e-8 & 6.1 && 2.89e-12 & 8.1  \\
  & $i = 5$ && 2.41e-6 & 4.1 && 2.27e-10 & 6.0 && 0 & $\infty$  \\
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 1$, NL}}
  & $i = 2$ && 5.52e0 & --- && 2.94e-2 & --- && 8.50e-5 & ---  \\
  & $i = 3$ && 5.71e-1 & 3.3 && 8.06e-3 & 5.2 && 6.09e-7 & 7.1  \\
  & $i = 4$ && 6.42e-2 & 3.2 && 2.36e-5 & 5.1 && 1.37e-11 & 15  \\
  & $i = 5$ && 7.61e-3 & 3.1 && 8.29e-10 & 15 && --- & ---  \\
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 2$, SI}}
  & $i = 2$ && 2.92e-2 & --- && 3.08e-2 & --- && \textcolor{red}{8.61e-6} & 7.3  \\
  & $i = 3$ && 7.68e-3 & 1.9 && 7.73e-3 & 1.9 && 6.18e-8 & 7.1  \\
  & $i = 4$ && \textcolor{red}{6.50e-3} & 3.2 && \textcolor{red}{2.39e-6} & 5.1 && 4.86e-4 & 5  \\
  & $i = 5$ && 7.71e-5 & 3.1 && 2.12e-3 & 5 && 4.86e-4 & 5  \\
  \midrule
  \multirow{4}{*}{\rotatebox[origin=c]{90}{$\globDim = 2$, NL}}
