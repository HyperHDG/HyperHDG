...
A comparison of the total errors can be conducted evaluating the errors of Tables \ref{TAB:diff_el_conv_ip} \& \ref{TAB:diff_hg_el_conv_ip} and comparing them to their LDG-H analogues, i.e.\ Tables \ref{TAB:diff_el_conv} \& \ref{TAB:diff_hg_el_conv}.
\begin{envarwarning}{Why is this so?!}
 Unfortunatelly, we did not manage to find a penalty $\tau := \tau(\locDim, \globDim, i, d, p)$ that gives appropriate convergence for $\locDim = 3$. In this case, the CG solver tends not to converge, and the $L^2$ error remains almost constant after some refinement steps.
\end{envarwarning}
% 
\subsection{Advection--diffusion--reaction problems}\label{SEC:other_solvers}
% 
