...
\end{equation*}
% 
\subsection{Existence and uniqueness of solutions}\label{SEC:ExUnique}\arnote{To be checked! It just flashed through my mind.}
% 
...
\begin{align}
 ...
 \tilde b(\mu) ~=~ & \int_\Node g_\textup N \mu \ds \label{EQ:lin_def}\\
...
% 
To construct the \emph{statically condensed mass matrix}, we consider \eqref{EQ:diffusion_mixed} in which $f$ as right-hand side of \eqref{EQ:div_is_f} is supposed to be equal to the unknown $u$, i.e.,
% 
\begin{subequations}\label{EQ:diffusion_fu_mixed}
\begin{align}
 \Div \vec q & = u && \text{ in } \Edge,\label{EQ:div_is_u}\\
 \tfrac{1}{d} \vec q + \Nabla u & = 0 && \text{ in } \Edge,\label{EQ:primal_dual_fu}\\
  u & = u_\textup D && \text{ on } \Node \in \SetNodeDir,\label{EQ:dir_cond_fu}\\
 \jump{\vec q} & = g_\textup N && \text{ on } \Node \in \SetNodeNeu,\label{EQ:neu_cond_fu}\\
 \jump{\vec q} & = 0 && \text{ on } \Node \in \SetNodeInt.\label{EQ:int_cond_fu}
\end{align}
\end{subequations}
% 
\begin{remark}\label{REM:ex_reaction}
 We do not consider the existence of possible solutions of this systems, but note that if $u$ (or $f$) is replaced by $- ru$ with some non-negative function $r$, the problem will again have an unique solution.
\end{remark}
The discretion of the local problems \eqref{EQ:hdg_scheme}, \eqref{EQ:hdg_f}, and global coupling condition \eqref{EQ:hdg_global} can be conducted exactly as prescribed in \eqref{EQ:hdg_global}. However, $f$ needs to be replaced by $U_\Edge$ in \eqref{EQ:hdg_f_flux} yielding the local problem
% 
...
\end{subequations}
% 
ultimately leading to the need to replace $\localU f$ and $\localQ f$ by $\localU (\localU \lambda)$ and $\localQ (\localU \lambda)$ in \eqref{EQ:construct_unk}, respectively:
% 
\begin{gather}\label{EQ:construct_unk_fu}
  \begin{split}
    U &= \localU \lambda + \localU u_\textup D + \localU (\localU \lambda),\\
    \vec Q &= \localQ \lambda + \localQ u_\textup D + \localQ (\localU \lambda).
  \end{split}
\end{gather}
% 
...
% 
\begin{align}
 \tilde a(\lambda, \mu) ~=~ & \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir}_{\node \in \edge} \int_\Node \left[ \localQ \lambda \cdot \Normal + \tau (\localU \lambda - \lambda) \right] \mu \ds,\label{EQ:stiff_def_fu}\\
 ...
 \tilde b(\mu) ~=~ & \int_\Node g_\textup N \mu \ds \label{EQ:lin_def_fu}
 - \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir}_{\node \in \edge} \int_\Node \left[ \localQ u_\textup D \cdot \Normal + \tau \localU u_\textup D \right] \mu \ds,
\end{align}
\end{subequations}
% 
...
% 
\begin{remark}
 The notation of $b$ is inconsistent, since it ``sometimes'' contains the right-hand side $f$ and ``sometimes'' it does not. The right-hand side $f$ can always be treated analogous to the aforemetioned procedure. Thus, neglecting $f$ for constructing the mass matrix is a simplification without loss of generality.
\end{remark}
% 
...
\begin{align}
 ...
 \jump{\vec q} & = g_\textup N = 0 && \text{ on } \Node \in \SetNodeNeu,\\
 \jump{\vec q} & = 0 && \text{ on } \Node \in \SetNodeInt,
\end{align}
\end{subequations}
% 
...
\end{equation}
% 
with $a$ and $\mass$ defined via \eqref{EQ:glob_system_old} and \eqref{EQ:forms}. Since we have, Remark \ref{REM:ex_reaction}, we can deduce that $\lambda$ needs to be positive to solve this problem.
...
\begin{align}
 ...
 \jump{\vec q} & = g_\textup N && \text{ on } (0,T) \times \Node \text{ with } \Node \in \SetNodeNeu,\\
 \jump{\vec q} & = 0 && \text{ on } (0,T) \times \Node \text{ with } \Node \in \SetNodeInt,
\end{align}
\end{subequations}
% 
...
\end{equation}
% 
with time-dependent $\mass$, $a$, and $b$. Applying some time discretization (preferably an implicit Euler scheme) gives an appropriate discretization if the initial $\lambda$ is done by $L^2$ projection to the skeletal space $\skeletalSpace$.
% 
\begin{remark}
 The equations mentioned in Remark \ref{REM:ex_reaction} can also be solved using our approach with slight modifications.
\end{remark}
% 
...
% 
\begin{equation*}
 -d \Delta u = v \qquad \text{ and } \qquad -\Delta v = f
\end{equation*}
% 
...
\begin{align}
 \Div \vec q & = \tfrac{1}{d} v && \text{ in } \Edge,\\
 \vec q + \Nabla u & = 0 && \text{ in } \Edge,\\
 u & = u_\textup D && \text{ on } \Node \in \SetNodeDir,\\
 \jump{\vec q} & = g_\textup N && \text{ on } \Node \in \SetNodeNeu,\\
 \jump{\vec q} & = 0 && \text{ on } \Node \in \SetNodeInt,\\
 \Div \vec r & = f && \text{ in } \Edge,\\
 \vec r + \Nabla v & = 0 && \text{ in } \Edge,\\
 v & = v_\textup D && \text{ on } \Node \in \SetNodeDir',\\
 \jump{\vec r} & = h_\textup N && \text{ on } \Node \in \SetNodeNeu',\\
 \jump{\vec r} & = 0 && \text{ on } \Node \in \SetNodeInt'.
\end{align}
\end{subequations}
% 
This way of formulating the problem already indicates that we will reuse the ideas Section \ref{SEC:hdg_graph} twice to discretize the bilaplacian. However, this might destroy the symmetry of the overall system.\arnote{The system converges poorly if CG is used.}

First, we define the local problem of a single hyperegde $\Edge$ assuming that functions $\lambda \in \skeletalSpace$ (approximating the trace of $u$) and $\eta \in \skeletalSpace$ (approximating the trace of $v$) are given. That is,
% 
\begin{subequations}\label{EQ:hdg_scheme_bilap}
 \begin{align}
  ...
  - \int_\Edge \vec Q_\Edge \cdot \Nabla w \dx  + \int_{\partial \Edge} ( \vec Q_\Edge \cdot \Normal + \tau  U_\Edge ) w \ds\notag\\
  - \int_\Edge \tfrac{1}{d} V_\Edge w \dx
  & = \tau \int_{\partial \Edge} \lambda w \ds \label{EQ:hdg_flux_bilap}\\
  \int_\Edge \vec R_\Edge \cdot \vec \varphi \dx - \int_\Edge V_\Edge \Div \vec \varphi \dx & = - \int_{\partial \Edge} \eta \vec \varphi \cdot \Normal \ds \label{EQ:hdg_primary_lap_bilap}\\
  - \int_\Edge \vec R_\Edge \cdot \Nabla \psi \dx  + \int_{\partial \Edge} ( \vec R_\Edge \cdot \Normal + \tau  V_\Edge ) \psi \ds
  & = \tau \int_{\partial \Edge} \eta \psi \ds \label{EQ:hdg_flux_lap_bilap}
 \end{align}
\end{subequations}
% 
This local solver induces $\localU(\lambda,\eta)$, $\localQ(\lambda,\eta)$, $\localV(\lambda,\eta)$, and $\localR(\lambda,\eta)$. Analogously to the diffusion's discretization, we also introduce
% 
\begin{subequations}\label{EQ:hdg_f_bilap}
 \begin{align}
  ...
  - \int_\Edge \vec Q_\Edge \cdot \Nabla w \dx  + \int_{\partial \Edge} ( \vec Q_\Edge \cdot \Normal + \tau  U_\Edge ) w \ds\notag\\
  - \int_\Edge \tfrac{1}{d} V_\Edge w \dx
  & = 0 \label{EQ:hdg_f_flux_bilap}\\
  \int_\Edge \vec R_\Edge \cdot \vec \varphi \dx - \int_\Edge V_\Edge \Div \vec \varphi \dx & = 0 \label{EQ:hdg_f_primary_lap_bilap}\\
  - \int_\Edge \vec R_\Edge \cdot \Nabla \psi \dx  + \int_{\partial \Edge} ( \vec R_\Edge \cdot \Normal + \tau  V_\Edge ) \psi \ds
  & = \int_{\Edge} f \psi \ds \label{EQ:hdg_f_flux_lap_bilap}
 \end{align}
\end{subequations}
% 
as the local problem related to $f$. The local problems which are related to Dirichlet datas are constructed analogously.
% 
% ---------------------------------------------------------------------
