...
\begin{gather}\label{EQ:construct_unk_fu}
  \begin{split}
    U &= \localU \lambda + \localU u_\textup D + \localU f + \localU (\localU \lambda + \localU u_\textup D + \localU f),\\
    \vec Q &= \localQ \lambda + \localQ u_\textup D + \localQ f + \localQ (\localU \lambda + \localU u_\textup D + \localU f).
  \end{split}
\end{gather}
% 
...
\begin{equation}\label{EQ:glob_system_old}
 \tilde a(\lambda, \mu) ~=~ \tilde b(\mu) - \left[ \mass (\lambda, \mu) + b^\mass(\mu) \right]
\end{equation}
% 
...
\begin{equation}\label{EQ:glob_system_new}
 a(\lambda, \mu) = b(\mu) + \mass(\lambda,\mu) + b^\mass(\mu),
\end{equation}
% 
...
\begin{table}
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  & $i = 3$ && 3.04e-4 & 2.2 && 3.90e-6 & 3.3 && \textcolor{red}{4.86e-4} & 5  \\
  & $i = 4$ && 7.08e-5 & 2.1 && \textcolor{red}{2.12e-3} & 5 && 4.86e-4 & 5  \\
  & $i = 5$ && 1.73e-5 & 2.0 && 2.12e-3 & 5 && 4.86e-4 & 5  \\
%   \midrule
%   \multirow{6}{*}{\rotatebox[origin=c]{90}{\textcolor{red}{$\globDim = 3$}}}
%   & $i = 0$ && 1.78e-2 & --- && 8.20e-3 & --- && 8.04e-4 & ---  \\
%   & $i = 1$ && 5.71e-3 & 5 && 5.57e-4 & 3.8 && 2.63e-5 & 4.9  \\
%   & $i = 2$ && 2.12e-3 & 5 && 4.25e-5 & 3.7 && 9.15e-7 & 4.8  \\
%   & $i = 3$ && 4.86e-4 & 5 && 4.03e-6 & 3.3 && 3.76e-8 & 4.6  \\
%   & $i = 4$ && 1.18e-4 & 5 && 4.54e-7 & 3.1 && 1.92e-9 & 4.2  \\
%   & $i = 5$ && 2.95e-5 & 5 && 5.51e-8 & 3.0 && 1.12e-10& 4.0  \\
%   \midrule
%   \multirow{6}{*}{\rotatebox[origin=c]{90}{\textcolor{red}{$\globDim = 4$}}}
%   & $i = 0$ && 1.78e-2 & --- && 8.20e-3 & --- && 8.04e-4 & ---  \\
%   & $i = 1$ && 5.71e-3 & 5 && 5.57e-4 & 3.8 && 2.63e-5 & 4.9  \\
%   & $i = 2$ && 2.12e-3 & 5 && 4.25e-5 & 3.7 && 9.15e-7 & 4.8  \\
%   & $i = 3$ && 4.86e-4 & 5 && 4.03e-6 & 3.3 && 3.76e-8 & 4.6  \\
...
%   & $i = 5$ && 2.95e-5 & 5 && 5.51e-8 & 3.0 && 1.12e-10& 4.0  \\
 ...
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc).}\label{TAB:diff_el_conv}
\end{table}
...
\begin{table}
 \begin{tabular}{c|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  \midrule
  $i = 0$ && 8.24e-2 & --- && \textcolor{red}{8.20e-3} & --- && \textcolor{red}{8.04e-4} & ---  \\
  $i = 1$ && 3.09e-2 & 1.4 && 5.57e-4 & 3.8 && 2.63e-5 & 4.9  \\
  $i = 2$ && 1.28e-2 & 1.2 && 4.25e-5 & 3.7 && 9.15e-7 & 4.8  \\
  $i = 3$ && 5.79e-3 & 1.1 && 4.03e-6 & 3.3 && 3.76e-8 & 4.6  \\
  $i = 4$ && 2.73e-3 & 1.0 && 4.54e-7 & 3.1 && 1.92e-9 & 4.2  \\
  $i = 5$ && \textcolor{red}{2.95e-5} & 2.0 && 5.51e-8 & 3.0 && 1.12e-10& 4.0  \\
  \bottomrule
 \end{tabular}\vspace{1ex}
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc) of linear approximation.}\label{TAB:diff_hg_el_conv}
\end{table}

Interestingly, the $L^2$ errors converge although with refinement, also the computational domain increases for $\locDim = 1$ and $\locDim = 2$.
...
\end{equation}
% 
with some initial datum $u_0$, $d = \tfrac{\pi}{2}$ such that $u = \sin(\tfrac{\pi}{2}(x + t))$ is the analytical solution in $\Omega = [0,1]^\globDim$. The results for an implicit Euler scheme with $\Delta t = 1e^-3$ can be found in Table \ref{TAB:diff_pb_conv}.
% 
\begin{table}
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  \toprule
...
%   & $i = 5$ && 2.95e-5 & 5 && 5.51e-8 & 3.0 && 1.12e-10& 4.0  \\
  \bottomrule
 \end{tabular}\vspace{1ex}
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc).}\label{TAB:diff_pb_conv}
...
\begin{subequations}
\begin{align}
 - a(\vec \lambda, \vec \mu) ~=~ & \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir^\dagger}_{\node \in \edge} \int_\Node \left[ \localQ \vec \lambda \cdot \Normal + \tau (\localU \vec \lambda - \lambda) \right] \bar \mu \ds\\
...
% 
\begin{align}
 \mass(\vec\lambda,\vec\mu) ~=~& \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir^\dagger}_{\node \in \edge} \int_\Node \left[ \localQ (\localU \vec\lambda) \cdot \Normal + \tau \localU (\localU \vec\lambda) \right] \bar \mu \ds \\
