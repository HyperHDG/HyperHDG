...
\end{subequations}
% 
where subscript $x$ identifies the longitudinal component (cf.~inner normal), and subscripts $y$ and $z$ the orthogonal components (cf.~outer normals) of the given quantities or derivatives, respectively. Equations  \eqref{EQ:Timo_2D_w} and \eqref{EQ:Timo_2D_phi} are sometimes tributed to S.~Timoshenko and P.~Ehrenfest if $\momentum_y \equiv 0$ is assumed.
...
Here, $A$, $E$, $G$, $I$ are material parameters. Their physical interpretation can be found in \cite[Sect.~1.3.3]{MeskourisH2013}. In the following considerations, they will be treated as constants and denoted by $c$.
...
\end{equation}
% 
where $(\tilde y, \tilde z)$ is the local coordinates of the line of motion of the axial load. Beyond that, the torque distribution $\momentum_x$ is related to the twist distribution $\torsion$ via
% 
\begin{equation}
 -\partial_x \bar c \partial_x \torsion_x = \momentum_x,
\end{equation}
% 
where the $c$ (possibly with subscript and/or bar) are material parameters.
