...
\begin{gather}\label{EQ:construct_unk_fu}
  \begin{split}
    U &= \localU \lambda + \localU u_\textup D + \localU f + \localU (\localU \lambda \textcolor{red}{+ \localU u_\textup D + \localU F}),\\
    \vec Q &= \localQ \lambda + \localQ u_\textup D + \localQ f + \localQ (\localU \lambda \textcolor{red}{+ \localU u_\textup D + \localU F}).
  \end{split}
\end{gather}\arerror{I believe this must be added! TODO: Check, why old version worked and implement this! Rewrite remainder!}
% 
...
\end{align}
% 
which represents the analogon to the right-hand side $b$ in \eqref{EQ:lin_def_bilap}. In our numerical experiments, we experienced that this is equivalent to writing $\mass$ as in \eqref{EQ:mass_theo}.
...
\begin{envarwarning}{State of implementation}
 ...
 The mass matrix representation given above does not work and I do not understand, why. The mass matrix, however does not seem to be far away.
