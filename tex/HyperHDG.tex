...
Here, we recognize that the achieved orders of convergence perfectly fit to the expected ones which are $p + 1$.
...
\begin{table}
 \begin{tabular}{c|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  \midrule
  $i = 0$ && \textcolor{red}{8.24e-2} & --- && 5.83e-2 & --- && 2.38e-2 & ---  \\
...
\begin{equation}\label{EQ:glob_system_old}
 a(\lambda, \mu) ~=~ b(\mu) + \left[ \bar \mass (\lambda, \mu) + \bar b^\mass(\mu) \right]
\end{equation}
% 
...
\end{equation*}
% 
saying that we seek a strangely normalized eigenfunction. Now the problem is square again, and we can try to solve it as some non-linear problem, i.e., using a fixed point iteration or Newton's method.
% 
\subsection{Fixed point iteration}
% 
For a fixed point iteration, we define $G$ for which we seek a fixed point, i.e. the pair $(\eigenval,\lambda)$, such that
% 
\begin{equation}
 G(\eigenval, \lambda) = (\eigenval,\lambda)^T,
\end{equation}
% 
where $G_\mu (\eigenval, \lambda) = f_\mu (\lambda, \eigenval) + \lambda_\mu$ and $G_0 (\eigenval, \lambda) = f_0 (\lambda) + \eigenval$.
% 
