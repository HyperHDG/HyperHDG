...
\begin{table}
 \begin{tabular}{cc|@{\,}lcc@{\,}lcc@{\,}lcc}
  ...
  & $i = 3$ && 3.04e-4 & 2.2 && 3.90e-6 & 3.3 && 3.63e-8 & 4.5  \\
  & $i = 4$ && 7.08e-5 & 2.1 && 4.49e-7 & 3.1 && 1.99e-9 & 4.1  \\
  & $i = 5$ && 1.73e-5 & 2.0 && 5.48e-8 & 3.0 && 1.59e-9 & 0.3  \\
  \bottomrule
 \end{tabular}\vspace{1ex}
 \caption{$L^2$ errors (err) and estimated orders of convergence (eoc) for elliptic example.}\label{TAB:diff_el_conv}
\end{table}

Here, we recognize that the achieved orders of convergence perfectly fit to the expected results. However, we used the actually suboptimal $\tau \sim \tfrac{1}{h}$ as compared to $\tau \sim 1$.
% 
\begin{envarnote}{$\tau$}
...
\subsection{An approximation to the statically condensed mass matrix}
% 
...
\begin{equation}\label{EQ:glob_system_old}
 \tilde a(\lambda, \mu) ~=~ \tilde b(\mu) - \left[ \bar \mass (\lambda, \mu) + \bar b^\mass(\mu) \right]
\end{equation}
% 
with bilinear and linear forms $\tilde a$ and $\tilde b$ as described in \eqref{EQ:forms_def}, and
% 
\begin{align}
 \bar \mass (\lambda,\mu) ~=~ & \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir}_{\node \in \edge} \int_\Node \left[ \localQ (\localU \lambda) \cdot \Normal + \tau \localU (\localU \lambda) \right] \mu \ds\label{EQ:mass_def}\\
 \bar b^\mass(\mu) ~=~ & \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir}_{\node \in \edge} \int_\Node \left[ \localQ (\localU f + \localU u_\textup D) \cdot \Normal + \tau \localU (\localU f + \localU u_\textup D) \right] \mu \ds
\end{align}
\end{subequations}
% 
of which $\bar \mass$ can be proven to be symmetric positive definite.
% 
\begin{envarerror}{Problems}
 This give the two $U_\Edge$ in the local solver different meanings. However it fits to the expected bilinear form. Moreover, it is highly doubtable, whether this is the accurate discretization for $p \ge 2$.
\end{envarerror}
% 
...
% 
\begin{equation}\label{EQ:glob_system_new}
 a(\lambda, \mu) = b(\mu) + \bar \mass(\lambda,\mu) + \bar b^\mass(\mu),
\end{equation}
% 
...
\end{equation}
% 
while the positive definiteness follows from the fact that $\bar \mass$ has, cf.~\cite[Thm.~2.1]{CockburnGL2009} with $d=-1$, 
% 
\begin{equation}\label{EQ:mass_theo}
 \bar \mass(\lambda,\mu) = \sum_{\edge \in \setEdge} \int_\Edge \localU \lambda \; \localU \mu \dx.
\end{equation}
% 
\begin{remark}
...
\end{remark}
% 
\subsection{Different types of solvable problems}\label{SEC:diff_types}
% 
...
% 
% ---------------------------------------------------------------------
\section{Parabolic diffusion equation}\arwarning{Only $\theta = 1$ has been implemented.}
% ---------------------------------------------------------------------
% 
Next, we solve the parabolic diffusion equation, i.e., we want that for all $\edge \in \setEdge$ and adjacent $\node \in \setNode$
% 
\begin{subequations}\label{EQ:diffusion_parab}
\begin{align}
 \tfrac{1}{d} \vec q + \Nabla u & = 0 && \text{ in } (0,T) \times \Edge,\label{EQ:primal_dual_parab}\\
 \partial_t u + \Div \vec q & = f && \text{ in } (0,T) \times \Edge,\label{EQ:div_is_f_parab}\\
 ...
 u(0,\cdot) & = u_0 && \text{ in } \Edge.\label{EQ:initial_parab}
\end{align}
\end{subequations}
% 
...
% 
% ---------------------------------------------------------------------
\section{Diffusive eigenvalue problem}
% ---------------------------------------------------------------------
% 
For the \emph{diffusive eigenvalue problem}, we seek a pair of a function  $u \neq 0$ and a real number $\eigenval \in \IR$, such that for all $\edge \in \setEdge$ and adjacent $\node \in \setNode$ the following set of equations holds:
% 
\begin{subequations}\label{EQ:diffusion_eigen}
\begin{align}
 \tfrac{1}{d} \vec q + \Nabla u & = 0 && \text{ in } \Edge,\\
...
\begin{subequations}\label{EQ:hdg_scheme_eigen}
 \begin{align}
  \int_\Edge \frac{1}{d} \vec Q_\Edge \cdot \vec p \dx - \int_\Edge U_\Edge \Div \vec p \dx & = - \int_{\partial \Edge} \lambda \vec p \cdot \Normal \ds \label{EQ:hdg_primary_parab}\\
  ...
  & = \tau \int_{\partial \Edge} \lambda v \ds \label{EQ:hdg_flux_parab}
 \end{align}
\end{subequations}
% 
...
% 
\begin{align*}
 f_\mu(\lambda,\eigenval) := & \sum_{\edge \in \setEdge} \sum^{\node \in \setNode \setminus \setNodeDir}_{\node \in \edge} \int_\Node \left[ \vec Q \cdot \Normal + \tau (U - \lambda) \right] \mu \ds \\
 = & \sum_{\node \in \setNode \setminus \setNodeDir} \int_\Node g_\textup N \mu \ds = 0.
\end{align*}
% 
...
% 
\begin{equation*}
 f_0 := \sum_{\node \in \setNode \setminus \setNodeDir} \int_\Node \lambda^2 \ds = 1
\end{equation*}
% 
saying that we seek a strangely normalized eigenfunction. Now the problem is square again, and we can try to solve it as some non-linear problem, i.e., using Newton's method. The Gateaux (directional) derivative of $f = (f_0,f_\mu)$ at $(\lambda, \eigenval)$ in direction $(\eta, \underline \eta)$ can be calculated as
% 
\begin{equation*}
...
% 
\begin{equation}
 \begin{pmatrix} \localQ'_\Edge(\underline \eta) \\ \localU'_\Edge (\underline \eta) \end{pmatrix} = - [\mathcal A(\eigenval)]^{-2} \begin{pmatrix} 0 & 0 \\ 0 & - \underline \eta \int_\Edge \varphi_j v_i \dx \end{pmatrix} \mathcal B(\lambda),
\end{equation}
\end{subequations}
% 
in the finite-dimensional sense, where the $\varphi_j$ make up a basis of $V$. This also indicates an appropriate implementation, while function valued arguments work analogously.\arwarning{To be checked.}
% 
% ---------------------------------------------------------------------
