 ...
 * 
 * This class administrates the mapping of topological information (i.e. a hypernode) to information related to
 * the approximation of solutions to PDEs. Thus, it is capable of mapping an hypernode index to the indices and
 * values of degrees of freedom. It is the only class to manage the access to the global vectors comprising
 * degrees of freedom and is universal to (almost) all kinds of possible equations.
 * 
 * @tparam	amount_of_local_dofs	Amount of degrees of freedom associated to an hypernode. This is the number
 * 									of local trial functions for the skeletal variable.
 * 
...
class HyperNodeFactory
{
  private:
	 ...
	 * 
	 * The number of hypernodes within the considered hypergraph is needed to construct vectors of the correct
	 * size, to check whether a vector has the appropriate size, and to check whether a degree of freedom has a
	 * valid index.
	 **********************************************************************************************************/
  ...
  public:
	 ...
	 * 
	 * @retval	num_of_global_dofs	The total amount of degreees of freedom in the considered hypergraph.
	 **********************************************************************************************************/
	 ...
	 * @retval	dof_indices		A @c std::array containing the global indices of related degrees of freedom.
	 **********************************************************************************************************/
	 ...
	 * 
	 * @param	hypernode_index	Index of the considered hypernode.
	 * @retval	dof_values		A @c std::array containing the values of related degrees of freedom.
	 **********************************************************************************************************/
    std::array<dof_value_type, amount_of_local_dofs> get_dof_values
      (const hypernode_index_type hypernode_index, const std::vector<dof_value_type>& global_dof_vector) const;
    /*!*********************************************************************************************************
     * @brief	Addy different values to values of degrees of freedom related to a hypernode.
	 * 
	 * Add local values of the @c std::array @c local_dof_vector to the respective values of the @c std::vector
	 * @c global_dof_vector comprising all degrees of freedom. 
	 * 
	 * @param	hypernode_index		Index of the considered hypernode.
	 * @param	global_dof_vector	@c std::vector containing the values of all degrees of freedom.
	 * @param   local_dof_vector	@c std::array containing the local values to be added to the global ones.
	 * @retval	global_dof_vector	@c std::vector containing the values of all degrees of freedom.
	 **********************************************************************************************************/
    void add_to_dof_values(const hypernode_index_type hypernode_index, std::vector<dof_value_type>&
      global_dof_vector, const std::array<dof_value_type, amount_of_local_dofs>& local_dof_vector) const;
    /*!*********************************************************************************************************
