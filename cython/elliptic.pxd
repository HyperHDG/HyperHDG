...
cdef extern from "<HyperHDG/global_loop/elliptic.hxx>" :
  cdef cppclass CythonClassName C++ClassName :
    ...
    vector[ CyReplace03 ] trace_to_flux (vector[ CyReplace03 ], CyReplace05 )
    vector[ CyReplace03 ] trace_to_flux (vector[ CyReplace03 ])
    vector[ CyReplace03 ] residual_flux (vector[ CyReplace03 ], CyReplace05 )
    vector[ CyReplace03 ] residual_flux (vector[ CyReplace03 ])
    vector[ CyReplace03 ] errors (vector[ CyReplace03 ], CyReplace05 )
    vector[ CyReplace03 ] errors (vector[ CyReplace03 ])
    vector[ CyReplace03 ] mean (vector[ CyReplace03 ], CyReplace05 )
    vector[ CyReplace03 ] mean (vector[ CyReplace03 ])
    int size_of_system ()
    string plot_option (string, string)
    void plot_solution (vector[ CyReplace03 ], CyReplace05 )
    void plot_solution (vector[ CyReplace03 ])
    unsigned int get_refinement()
    void set_refinement(unsigned int)
    sparse_mat trace_to_flux_mat( CyReplace05 )
