 ...
 **************************************************************************************************/
std::vector<std::string>& fill_file_names(std::vector<std::string>& include_files,
 ...
 **************************************************************************************************/
string cythonize(vector<string>& names,
                 const vector<string>& filenames,
                 const unsigned int ver_maj,
                 const unsigned int ver_min,
                 const bool debug_mode = false)
{
  ...
  if (python_name != "hyper_cythonizer" && debug_mode)
    python_name.append("_deb");
  else if (python_name != "hyper_cythonizer")
    python_name.append("_rel");
...

  {
    ...
    for (unsigned int i = 0; i < filenames.size(); ++i)
      if (filenames[i].substr(0, 8) == "HyperHDG")
        ...
        file_names.push_back(fs::current_path().string() + "/" + filenames[i]);
    for (unsigned int i = 0; i < file_names.size(); ++i)
    {
      cxx_file = file_names[i];
      hy_assert(exists(cxx_file), "Included file " << cxx_file << " needs to exist!");
      if (so_time < fs::last_write_time(cxx_file))
        goto do_compilation;
...
do_compilation:  // Needed for goto!
  ...
  while (std::getline(infile, line))
  {
    ...
    while (!linestream.eof() && !line.empty())
    {
      ...
      if (word == "C++ClassName")
        word = "\"" + names[1] + "\"";
      else if (word == "CythonClassName")
        word = python_name + "_Cython";
      else if (word == "PythonClassName")
        word = python_name;
      else if (word == "#")
      {
        outfile << word << " ";
        ...
        if (word == "CyReplace_Number:")
        {
          ...
          hy_assert(replace_number + 3 > names.size() && replace_number <= 99,
                    "The number of rplacements must be larger than their given amount."
                      << std::endl
                      << "Replace number = " << replace_number << ". Names size = " << names.size()
                      << ".");
          names.resize(replace_number + 2);
        }
        else if (word.size() == 12 && word.substr(0, 9) == "CyReplace" &&
                 word.substr(11, 12) == ":")
        {
          unsigned int replace_number = stoi(word.substr(9, 11));
          hy_assert(
            names.size() > replace_number + 1 && replace_number >= 1 && replace_number <= 99,
            "The vector names must contain enough entries to ensure proper replacements "
              << "and the replace number must be in [01,99].");
          outfile << word << " ";
          linestream >> word;
          if (names[replace_number + 1] == "")
            names[replace_number + 1] = word;
        }
      }
