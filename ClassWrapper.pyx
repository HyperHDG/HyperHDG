...
cdef class PyDiffusionProblemNaiveF:
  cdef DiffusionProblemNaiveF *thisptr      # hold a C++ instance which we're wrapping
  def __cinit__(self, num_elements):
    self.thisptr = new DiffusionProblemNaiveF(num_elements, num_elements, 1.)
  def __dealloc__(self):
    del self.thisptr
  def read_dirichlet_indices(self, indices):
    self.thisptr.read_dirichlet_indices(indices)
...
"""
cdef class PyElasticityBBeam:
  cdef ElasticityBBeam *thisptr      # hold a C++ instance which we're wrapping
  def __cinit__(self, filename):
    if isinstance(filename,str): # Python3 version - use unicode for Python 2
      filename = filename.encode()
    self.thisptr = new ElasticityBBeam(filename, 1.)
  def __dealloc__(self):
    del self.thisptr
  def read_dirichlet_indices(self, indices):
    self.thisptr.read_dirichlet_indices(indices)
