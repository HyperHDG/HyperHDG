# --------------------------------------------------------------------------------------------------
# Python functions to write a star_{level}_{dimension}_fdir.geo -file
# containing the domain information for HyperHDG
# The domain, compared to the "star_shape_functions", has full dirichlet boundaries instead of the "end points"
...
def create_star_geo(dimension, level, filename):
  ...
  # Initialize the matrix as zeros of size nodesOfHyEdges
  typesOfHyFaces = np.zeros(nodesOfHyEdges.shape, dtype=int)

  # Build the matrix itself:
  values, counts = np.unique(nodesOfHyEdges, return_counts=True)
  for i in range(0, len(values)):
    if (counts[i] == 1):
      typesOfHyFaces = np.where(nodesOfHyEdges != values[i], typesOfHyFaces, 1)
...
def get_star_domain(dimension, level):
  filename = 'star_' + str(dimension) + '_' + str(level) + '_fdir.geo'
