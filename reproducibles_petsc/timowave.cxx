...
template <template <unsigned int, typename> typename Test>
static PetscErrorCode CreateDeg(PetscInt poly_deg,
                                const char* path,
                                PetscReal tau,
                                PetscReal theta,
                                PetscReal dt,
                                HDGBase** hdg)
{
  ...
  {
    case 1:
      *hdg = new HDGWrapper(HDGTimoWave<1, Test>(path, {tau, theta, dt}));
      break;
    case 2:
      *hdg = new HDGWrapper(HDGTimoWave<2, Test>(path, {tau, theta, dt}));
      break;
    case 3:
      *hdg = new HDGWrapper(HDGTimoWave<3, Test>(path, {tau, theta, dt}));
      break;
    case 6:
      *hdg = new HDGWrapper(HDGTimoWave<6, Test>(path, {tau, theta, dt}));
      break;
    default:
      PetscCheck(false, PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "unsupported poly_deg = %d",
                 (int)poly_deg);
  }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode PetscHDGCreate(PetscInt poly_deg,
                              const char* test,
                              const char* path,
                              PetscReal tau,
                              PetscReal theta,
                              ...
                              HDGBase** hdg)
{
  ...
  if (0 == strcmp(test, "stiffness"))
    PetscCall(CreateDeg<TimoshenkoStiffness>(poly_deg, path, tau, theta, dt, hdg));
  else if (0 == strcmp(test, "sinclamp"))
    PetscCall(CreateDeg<TimoshenkoSinClamp>(poly_deg, path, tau, theta, dt, hdg));
  else if (0 == strcmp(test, "gaussian"))
    PetscCall(CreateDeg<TimoshenkoGaussian>(poly_deg, path, tau, theta, dt, hdg));
  else if (0 == strcmp(test, "wave4"))
    PetscCall(CreateDeg<TestTimoWave4>(poly_deg, path, tau, theta, dt, hdg));
  else
    PetscCheck(false, PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "unknown test = \"%s\"", test);
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
int main(int argc, char** argv)
{
  ...
  PetscCall(PetscOptionsReal("-theta",
                             "time-step averaging weight, 0 < theta <= 0.5, use theta=0.25 for CN",
                             NULL, theta, &theta, &is_set));
  ...
  PetscCall(PetscOptionsString("-plot", "plot solution using HyperHGD", NULL, plot, plot, PATH_MAX,
                               &is_set));
  PetscCall(PetscOptionsString("-mat_cache", "path to matrix cache", NULL, mat_cache, mat_cache,
                               PATH_MAX, &is_set));
  PetscCall(PetscOptionsString("-static", "path static init trace variables", NULL, static_init,
                               static_init, PATH_MAX, &is_set));
  PetscCall(PetscOptionsString("-domain", "domain path", NULL, domain_path, domain_path, PATH_MAX,
                               &is_set));
  PetscCall(PetscOptionsString("-test", "timowave test problem: stiffness, wave1, wave4", NULL,
                               timowave_test, timowave_test, sizeof(timowave_test), &is_set));
  PetscCall(PetscOptionsBool("-print_timestep", "print timestep progress", NULL, print_timestep,
                             &print_timestep, &is_set));
  PetscCall(
    PetscOptionsBool("-mat_only", "only compute matrix", NULL, mat_only, &mat_only, &is_set));
