...
{
/*!*************************************************************************************************
 * \brief   Combine local solver and global information for the CH-KP equation.
 *
...
class Chkp
{
   ...
   * \brief   Prepare struct to check for function to exist (cf. compile_time_tricks.hxx).
   ************************************************************************************************/
  HAS_MEMBER_FUNCTION(make_initial_skeleton, has_make_initial_skeleton);
  /*!***********************************************************************************************
   * \brief   Prepare struct to check for function to exist (cf. compile_time_tricks.hxx).
   ************************************************************************************************/
  ...
  LargeVecT make_initial(const LargeVecT& x_vec, const dof_index_t time = 0.)
  {
    ...
    std::for_each(
      ...
      [&](auto hyper_edge)
      {
        ...
        // Turn degrees of freedom of x_vec that have been stored locally into those of vec_Ax.
        if constexpr (has_make_initial_skeleton<LocalSolverT,
                                       std::array<std::array<dof_value_t, n_dofs_per_node>,
                                         ...
                                         dof_value_t)>::value)
        {
          local_solver_.make_initial_skeleton(hyEdge_dofs, time);
        }
        else if constexpr (has_make_initial_skeleton<LocalSolverT,
                                            std::array<std::array<dof_value_t, n_dofs_per_node>,
                                                       2 * TopologyT::hyEdge_dim()>&(
                                              std::array<std::array<dof_value_t, n_dofs_per_node>,
                                                         2 * TopologyT::hyEdge_dim()>&,
                                              decltype(hyper_edge)&, dof_value_t)>::value)
        {
          local_solver_.make_initial_skeleton(hyEdge_dofs, hyper_edge, time);
        }
        else
          hy_assert(false, "Function seems not to be implemented!");
