    ...
    std::for_each(hyper_graph_.begin(), hyper_graph_.end(),
                  [&](auto hyper_edge)
                  {
                    ...
                    for (unsigned int i = 0; i < hyNode_types.size(); ++i)
                      if (hyNode_types[i] == 1)
                      {
                        hyper_graph_.hyNode_factory().get_dof_indices(hyEdge_hyNodes[i],
                                                                      dof_indices);
  ...
  LargeVecT trace_to_mass_flux(const LargeVecT& x_vec, const dof_value_t time = 0.)
  {
    ...
    std::for_each(
      ...
      [&](auto hyper_edge)
      {
        ...
        // Turn degrees of freedom of x_vec that have been stored locally into those of vec_Ax.
        if constexpr (requires { local_solver_.trace_to_mass_flux(hyEdge_dofs_old, hyEdge_dofs_new, prvalue_of(time)); })
          local_solver_.trace_to_mass_flux(hyEdge_dofs_old, hyEdge_dofs_new, time);
        else if constexpr (requires { local_solver_.trace_to_mass_flux(hyEdge_dofs_old, hyEdge_dofs_new, hyper_edge, prvalue_of(time)); })
          local_solver_.trace_to_mass_flux(hyEdge_dofs_old, hyEdge_dofs_new, hyper_edge, time);
        else
          static_assert(always_false_v<LocalSolverT, decltype(hyper_edge)>,
                        "LocalSolverT implements no usable overload of trace_to_mass_flux!");
