...
template <unsigned int hyEdge_dimT,
          ...
          typename std::array<double, space_dimT> scalesT,
...
class Rectangle
{
  ...
  class hyEdge
  {
   private:
    ...
    unsigned int fill_data(unsigned int index,
                           ...
                           const std::array<double, space_dimT> scalesTT)
    {
        ...
        if (index == 0)
          for (unsigned int dim_pt = 0; dim_pt < space_dimT; ++dim_pt)
          {
            ...
            translation[ext_dim] = (pt_coord_t)Wrapper::exterior_coordinate(elem, dim_pt) /
                                   (pt_coord_t)geometry.n_elements_[ext_dim] * scalesTT[dim_pt];
      ...
      else
      {
        for (unsigned int i = 2 * hyEdge_dimTT - 2; i < 2 * hyEdge_dimTT; ++i) {
          index = fill_data<hyEdge_dimTT - 1>(index, Wrapper::get_face(elem, i), geometry, scalesTT);
        }
      }
      return index;
    }
   ...
   public:
     ...
     **********************************************************************************************/
    hyEdge(const hyEdge_index_t index, const Rectangle& geometry)
    {
      ...
      fill_data<hyEdge_dimT>(0, elem, geometry, scalesT);
 ...
 public:
   ...
   *
   * The \c class \c Rectangle defines the geometry of the hypergraph. It contains the different
   ...
   ************************************************************************************************/
  Rectangle(const constructor_value_type& n_elements)
   ...
   ************************************************************************************************/
  Rectangle(const Topology::Cubic<hyEdge_dimT, space_dimT>& other)
