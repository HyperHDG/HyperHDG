...
class LengtheningBeam
{
 ...
 private:
                                            ...
                                            hyEdgeT& hyper_edge) const
  {
    ...
    for (unsigned int i = 0; i < 2 * hyEdge_dimT; ++i)
      for (unsigned int dim = 0; dim < space_dim; ++dim)
        lambda_values_out[i][space_dim + dim] += normal_vector[dim] * lambda[i][0];
...
class TwistingBeam
{
 ...
 public:
   ...
   * \param   tau           Penalty parameter of HDG scheme.
   ************************************************************************************************/
  TwistingBeam(const constructor_value_type& tau = 0.) : diffusion(tau) {}
  /*!***********************************************************************************************
   * \brief   Evaluate local contribution to matrix--vector multiplication.
   *
   * \tparam  hyEdgeT             The geometry type / typename of the considered hyEdge's geometry.
...
class LengtheningBernoulliBendingBeam
{
 ...
 public:
  ...
  : len_beam(tau), twist_beam(tau), ben_beam(tau)
  {
  }
  /*!***********************************************************************************************
   * \brief   Evaluate local contribution to matrix--vector multiplication.
                              ...
                              const lSol_float_t time = 0.) const
  {
    ...
    twist_beam.trace_to_flux(lambda_values_in, lambda_values_out, hyper_edge, time);
                              ...
                              const lSol_float_t time = 0.) const
  {
    ...
    twist_beam.residual_flux(lambda_values_in, lambda_values_out, hyper_edge, time);
