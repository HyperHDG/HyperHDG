...
class DiffusionWave1
{
 ...
 public:
   ...
   ************************************************************************************************/
  DiffusionWave1(const constructor_value_type& constru = std::vector(3, 1.))
      ...
      for (unsigned int j = 0; j < n_shape_bdr_; ++j)
        for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
        {
          auto integral = integrator::template integrate_bdr_phipsi<decltype(hyEdgeT::geometry)>(
            i, j, face, hyper_edge.geometry);
          hyper_edge.data.flux_old[i] += tau_ * lambda_values[face][j] * integral;
          for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
            hyper_edge.data.qflux_old[dim * n_shape_fct_ + i] -=
      ...
      for (unsigned int i = 0; i < n_shape_fct_; ++i)
      {
        q_components[dim][i] = hyper_edge.data.q_old[dim * n_shape_fct_ + i] =
          integrator::template integrate_volUni_phifunc<
            Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>,
            decltype(hyEdgeT::geometry), parameters::initial_q>(i, hyper_edge.geometry, time);
      }
    }
...
// -------------------------------------------------------------------------------------------------
//
// IMPLEMENTATION OF MEMBER FUNCTIONS OF DiffusionWave1
//
// -------------------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------------------
...
inline SmallSquareMat<
  DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_loc_dofs_,
  lSol_float_t>
DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::assemble_loc_matrix(
  ...
  const lSol_float_t time) const
{
  ...
  for (unsigned int i = 0; i < n_shape_fct_; ++i)
  {
    for (unsigned int j = 0; j < n_shape_fct_; ++j)
    {
      ...
      for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
      {
        ...
        local_mat(dim * n_shape_fct_ + i, hyEdge_dimT * n_shape_fct_ + j) -=
  ...
  return local_mat;
}  // end of DiffusionWave1::assemble_loc_matrix
...
inline SmallVec<
  DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_loc_dofs_,
  lSol_float_t>
DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::
      ...
      for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
      {
        ...
        for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
          right_hand_side[dim * n_shape_fct_ + i] -=
            hyper_edge.geometry.local_normal(face).operator[](dim) * lambda_values[face][j] *
            integral * delta_t_ * theta_;
      }

  return right_hand_side;
}  // end of DiffusionWave1::assemble_rhs_from_lambda
  ...
  for (unsigned int i = 0; i < n_shape_fct_; ++i)
  {
    ...
    for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
    {
      ...
      integral += (1. - theta_) * integrator::template integrate_bdr_phifunc<
                                    Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>,
                                    decltype(hyEdgeT::geometry), parameters::dirichlet_value,
                                    Point<hyEdge_dimT, lSol_float_t> >(i, face, hyper_edge.geometry,
                                                                       time - delta_t_);
  ...
  for (unsigned int i = 0; i < n_shape_fct_ * hyEdge_dimT; ++i)
    right_hand_side[i] += hyper_edge.data.q_old[i] *
                            integrator::template integrate_vol_phiphifunc<
                              ...
                              Point<hyEdge_dimT, lSol_float_t> >(i, i, hyper_edge.geometry, time) +
    ...
    hyEdgeT& hyper_edge) const
{
  ...
  return right_hand_side;
}  // end of DiffusionWave1::assemble_rhs_from_coeffs
...
inline std::array<
  std::array<
    lSol_float_t,
    DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_shape_bdr_>,
  2 * hyEdge_dimT>
DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::primal_at_boundary(
  const std::array<lSol_float_t, n_loc_dofs_>& coeffs,
  hyEdgeT& hyper_edge) const
{
  ...
  return bdr_values;
}  // end of DiffusionWave1::primal_at_boundary
...
inline std::array<
  std::array<
    ...
    DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_shape_bdr_>,
  2 * hyEdge_dimT>
DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::dual_at_boundary(
  const std::array<lSol_float_t, (hyEdge_dimT + 1) * n_shape_fct_>& coeffs,
  hyEdgeT& hyper_edge) const
{
  std::array<std::array<lSol_float_t, n_shape_bdr_>, 2 * hyEdge_dimT> bdr_values;
  ...
  return bdr_values;
}  // end of DiffusionWave1::dual_at_boundary
...
std::array<std::array<lSol_float_t, Hypercube<hyEdge_dimT>::pow(abscissas_sizeT)>,
           DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::system_dim>
DiffusionWave1<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::bulk_values(
...
// end of DiffusionWave1::bulk_values
