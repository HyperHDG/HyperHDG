...
struct BeamNetworkDiffusionParametersDefault
{
  /*!***********************************************************************************************
...
struct BeamNetworkDiffusionParametersTwist
{
  ...
  {
    // return 0.;
    // return 12. * point[0] * point[0] * (normal[0] > 0.);
    // return M_PI * M_PI * sin(M_PI * point[0]) * normal[0];
    return - M_PI * M_PI * M_PI * cos(M_PI * point[0]) * (normal[0] > 0.);
  }
  /*!***********************************************************************************************
   * \brief   Dirichlet values of solution as analytic function.
   ************************************************************************************************/
  ...
  {
    // return 0.;
    // return point[0] * point[0] * point[0] * point[0] * (normal[0] > 0.);
    return - M_PI *  cos(M_PI * point[0]) * (normal[0] > 0.);
    // return sin(M_PI * point[0]) * normal[0];
  }
};  // end of struct DiffusionParametersDefault
...
class BeamNetworkDiffusion
{
 ...
 public:
   ...
   ************************************************************************************************/
  BeamNetworkDiffusion(const constructor_value_type& tau = 1.) : tau_(tau) {}
  /*!***********************************************************************************************
                                      ...
                                      const lSol_float_t time = 0.) const
  {
    ...
    return std::array<lSol_float_t, 1U>({integrator::template integrate_vol_diffsquare_discanacomp<
      Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>, decltype(hyEdgeT::geometry),
      parameters::analytic_result, Point<hyEdge_dimT, lSol_float_t> >(coeffs, 2, hy_edge.geometry,
                                                                      time)});
  }
  /*!***********************************************************************************************
  ...
  for (unsigned int i = 0; i < n_shape_fct_; ++i)
  {
    ...
    for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
    {
      ...
      integral = integrator::template integrate_bdr_phivecfunccomp<
        Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>, decltype(hyEdgeT::geometry),
        parameters::dirichlet_value, Point<hyEdge_dimT, lSol_float_t> >(i, face, 2,
