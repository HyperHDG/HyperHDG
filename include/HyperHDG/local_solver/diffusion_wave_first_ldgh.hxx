...
class DiffusionWaveFirst
{
 ...
 public:
   ...
   ************************************************************************************************/
  struct ufb_type {
    SmallVec<n_shape_fct_, lSol_float_t> u, flux;
    SmallMat<n_shape_bdr_, 2 * hyEdge_dimT, lSol_float_t> boundary_flux;
  };
  using data_type = std::array<ufb_type, 2>;
  /*!***********************************************************************************************
   ...
   ************************************************************************************************/
  DiffusionWaveFirst(const constructor_value_type& constru = std::vector(3, 1.))
  ...
  SmallMatOutT& trace_to_flux(SmallMatInT& lambda_values_in,
    ...
    const lSol_float_t time = 0.) const
  {
    ...
    {
      // f v
      hyper_edge.data[0].flux[i] = integrator::template integrate_vol_phifunc<
      ...
      for (unsigned int j = 0; j < n_shape_fct_; ++j)
      {
        ...
        for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
        {
            ...
            for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
              hyper_edge.data[0].flux[i] -= q_components[dim][j] *
    ...
    for (unsigned int i = 0; i < lambda_values.size(); ++i)
    {
      if (is_dirichlet<parameters>(hyper_edge.node_descriptor[i]))
        for (unsigned int j = 0; j < lambda_values[i].size(); ++j)
          hyper_edge.data[0].boundary_flux(j, i) = 0.;
      else
        for (unsigned int j = 0; j < lambda_values[i].size(); ++j)
          hyper_edge.data[0].boundary_flux(j, i) =
            duals[i][j] + tau_ * primals[i][j] -
                          ...
                          const lSol_float_t time = 0.) const
  {
    ...
    auto fill_data = [&](ufb_type& ufb, lSol_float_t t) {
      ...
      if (is_dirichlet<parameters>(hyper_edge.node_descriptor[i]))
        ...
        for (unsigned int j = 0; j < lambda_values[i].size(); ++j)
          lambda_values[i][j] = integrator::template integrate_bdrUni_psifunc<
            Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>,
            decltype(hyEdgeT::geometry), parameters::initial>(j, i, hyper_edge.geometry, t);
    }

    // Define primary as L^2 projection!
    for (unsigned int i = 0; i < n_shape_fct_; ++i)
      ufb.u[i] = integrator::template integrate_volUni_phifunc<
        Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>, decltype(hyEdgeT::geometry),
        parameters::initial>(i, hyper_edge.geometry, t);
    ...
    for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
    {
      for (unsigned int i = 0; i < n_shape_fct_; ++i)
      {
        local_rhs[i] = integrator::template integrate_vol_derphifunc<
          ...
          i, dim, hyper_edge.geometry, t);
        for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
          local_rhs[i] -=
            integrator::template integrate_bdr_phifunc<
              Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>,
              decltype(hyEdgeT::geometry), parameters::initial, Point<hyEdge_dimT, lSol_float_t> >(
              i, face, hyper_edge.geometry, t) *
            hyper_edge.geometry.local_normal(face).operator[](dim);
      }
    ...
    for (unsigned int i = 0; i < n_shape_fct_; ++i)
    {
      ufb.flux[i] = integrator::template integrate_vol_phifunc<
        Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>, decltype(hyEdgeT::geometry),
      ...
      for (unsigned int j = 0; j < n_shape_fct_; ++j)
      {
        grad_int_vec =
          integrator::template integrate_vol_nablaphiphi<Point<hyEdge_dimT, lSol_float_t>,
            ...
            i, j, hyper_edge.geometry);
        for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
          ufb.flux[i] += q_components[dim][j] * grad_int_vec[dim];
        for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
        {
          helper = integrator::template integrate_bdr_phiphi<decltype(hyEdgeT::geometry)>(
            i, j, face, hyper_edge.geometry);
          for (unsigned int dim = 0; dim < hyEdge_dimT; ++dim)
            ufb.flux[i] -= q_components[dim][j] *
                                           hyper_edge.geometry.local_normal(face).operator[](dim) *
                                           helper;
          ufb.flux[i] -= tau_ * ufb.u[j] * helper;
        }
      }
      for (unsigned int j = 0; j < n_shape_bdr_; ++j)
        for (unsigned int face = 0; face < 2 * hyEdge_dimT; ++face)
    ...
    for (unsigned int i = 0; i < n_shape_fct_; ++i)
      coeffs[hyEdge_dimT * n_shape_fct_ + i] = ufb.u[i];
    std::array<std::array<lSol_float_t, n_shape_bdr_>, 2 * hyEdge_dimT> primals(
      primal_at_boundary(coeffs, hyper_edge)),
      duals(dual_at_boundary(coeffs, hyper_edge));
    ...
    for (unsigned int i = 0; i < lambda_values.size(); ++i)
    {
      if (is_dirichlet<parameters>(hyper_edge.node_descriptor[i]))
        for (unsigned int j = 0; j < lambda_values[i].size(); ++j)
          ufb.boundary_flux(j, i) = 0.;
      else
        for (unsigned int j = 0; j < lambda_values[i].size(); ++j)
          ufb.boundary_flux(j, i) =
            duals[i][j] + tau_ * primals[i][j] -
            tau_ * lambda_values[i][j] * hyper_edge.geometry.face_area(i);
    }
    ...
    fill_data(hyper_edge.data[0], time);
    fill_data(hyper_edge.data[1], time-delta_t_); // HACK: data[1].u could be filled by using initial dt, but how could q?
                                      ...
                                      const lSol_float_t time = 0.) const
  {
    ...
    return std::array<lSol_float_t, 1U>({integrator::template integrate_vol_diffsquare_discana<
      Point<decltype(hyEdgeT::geometry)::space_dim(), lSol_float_t>, decltype(hyEdgeT::geometry),
      parameters::analytic_result, Point<hyEdge_dimT, lSol_float_t> >(hy_edge.data[0].u.data(),
...
// -------------------------------------------------------------------------------------------------
//
// IMPLEMENTATION OF MEMBER FUNCTIONS OF DiffusionWaveFirst
//
// -------------------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------------------
...
inline SmallSquareMat<
  DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_loc_dofs_,
  lSol_float_t>
DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::assemble_loc_matrix(
  ...
  const lSol_float_t time) const
{
  ...
  return local_mat;
}  // end of DiffusionWaveFirst::assemble_loc_matrix
...
inline SmallVec<
  DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_loc_dofs_,
  lSol_float_t>
DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::
...
}  // end of DiffusionWaveFirst::assemble_rhs_from_lambda
  ...
  assemble_rhs_from_global_rhs(hyEdgeT& hyper_edge, const lSol_float_t time) const
{
  ...
  return right_hand_side;
}  // end of DiffusionWaveFirst::assemble_rhs_from_global_rhs
    ...
    hyEdgeT& hyper_edge) const
{
  ...
  return right_hand_side;
}  // end of DiffusionWaveFirst::assemble_rhs_from_coeffs
...
inline std::array<
  std::array<
    lSol_float_t,
    DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_shape_bdr_>,
  2 * hyEdge_dimT>
DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::primal_at_boundary(
  const std::array<lSol_float_t, n_loc_dofs_>& coeffs,
  hyEdgeT& hyper_edge) const
{
  ...
  return bdr_values;
}  // end of DiffusionWaveFirst::primal_at_boundary
...
inline std::array<
  std::array<
    ...
    DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::n_shape_bdr_>,
  2 * hyEdge_dimT>
DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::dual_at_boundary(
  const std::array<lSol_float_t, (hyEdge_dimT + 1) * n_shape_fct_>& coeffs,
  hyEdgeT& hyper_edge) const
{
  std::array<std::array<lSol_float_t, n_shape_bdr_>, 2 * hyEdge_dimT> bdr_values;
  ...
  return bdr_values;
}  // end of DiffusionWaveFirst::dual_at_boundary
...
std::array<std::array<lSol_float_t, Hypercube<hyEdge_dimT>::pow(abscissas_sizeT)>,
           DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::system_dim>
DiffusionWaveFirst<hyEdge_dimT, poly_deg, quad_deg, parametersT, lSol_float_t>::bulk_values(
...
// end of DiffusionWaveFirst::bulk_values
