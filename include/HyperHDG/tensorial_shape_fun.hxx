...
class TensorialShapeFunctionEvaluation {
 private:
  ...
  inline void generate_values_for_abscissas (const std::array<abscissa_float_t, abscissas_sizeT> abscissas)
  {
 ...
 public:
  ...
  unsigned int get_shapefunction_count ()
  {
	return shapefunction_count;
  }
  inline return_t
  evaluate_in_tensorial_point (unsigned int tensorial_function_index, unsigned int tensorial_point_index)
  {
	std::array<unsigned int, std::max (dimT, 1U)> decomposed_function_index = index_decompose<dimT,
																							  polynomial_degree
																							  + 1> (tensorial_function_index);
	std::array<unsigned int, std::max (dimT, 1U)> decomposed_point_index = index_decompose<dimT,
																						   abscissas_sizeT> (tensorial_point_index);
...
/*!*********************************************************************************************
 * \brief   Evaluate a multidimensional linear combination of tensor product shapefunctions in tensor product points
 *
 * \param linear_combination_coefficients flat array of the coefficients of the linear combination
 **********************************************************************************************/
  template<unsigned int coefficient_dimension>
  inline std::array<std::array<return_t, point_count>, coefficient_dimension>
  evaluate_linear_combination_in_all_tensorial_points (std::array<return_t,
	...
	for(unsigned int function = 0; function < shapefunction_count; ++function) {
		for (unsigned int point = 0; point < point_count; ++point) {
		  function_value = evaluate_in_tensorial_point(function, point);
			for (unsigned int d = 0; d < coefficient_dimension; ++d) {
				return_values[d][point] += function_value*linear_combination_coefficients[d*shapefunction_count+function];
			  }
		  }
	  }
