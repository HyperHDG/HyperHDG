...
def get_options():
  opt = options()
  if not os.path.isfile(main_dir() + "/build/cmake_cython.cfg") and not os.path.isfile("/github/workspace/build/cmake_cython.cfg"):
    ...
    if os.path.isfile(main_dir() + "/build/cmake_cython.cfg"):
      ...
      config.read("/github/workspace/build/cmake_cython.cfg")
