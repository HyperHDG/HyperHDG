...
# Use ANSI-C quoting ($'...') so the escape characters are stored literally. This is portable
# across the bash that ships with Linux and the older bash 3.2 on macOS, whose 'echo -e' does not
# understand the '\e' shorthand and would otherwise print the raw escape codes.
COL=$'\033[0;36m'  # text format 3 -> 'text color', 6 -> 'cyan'
NOR=$'\033[0m'     # text format 'standard'
...
while getopts "cCdD" opt; do
  case $opt in
    ...
    (D) ynD=y;;
    (d) ynD=n;;
    (*) echo "${COL}Illegal option set.${NOR}" && exit 1;;
  esac
done

input_correct=false
if [ -z "$ynC" ]; then
  read -p "${COL}Do you wish to build HyperHDG on your computer? [Yn] ${NOR}" ynC
fi
while [ "$input_correct" = false ]; do
  if [ -z "$ynC" ]; then
    ...
    case $ynC in
      ...
      [Nn] | [Nn]o  ) input_correct=true && echo "${COL}Skipping installation ...${NOR}";;
      *             ) read -p "${COL}Please answer yes or no. ${NOR}" ynC;;
    esac
  fi
done

echo " "
echo "${COL}You might need to be root to build HyperHDG within a Docker container.${NOR}"
input_correct=false
if [ -z "$ynD" ]; then
   read -p "${COL}Do you want to build HyperHDG in a Docker? [yN] ${NOR}" ynD
fi
while [ "$input_correct" = false ]; do
  if [ -z "$ynD" ]; then
     input_correct=true && echo "${COL}Skipping installation ...${NOR}"
  else
    case $ynD in
      [Yy] | [Yy]es ) input_correct=true && setup_docker;; 
      [Nn] | [Nn]o  ) input_correct=true && echo "${COL}Skipping installation ...${NOR}";;
      *             ) read -p "${COL}Please answer yes or no. ${NOR}" ynD;;
    esac
  fi
done
