...
#define PetscArraycpyCast(dst, src, n, dsttype, srctype) \
  ...
  {                                                      \
    for (typeof(n) _i = 0; _i < (n); _i++)               \
      (dst)[_i] = (dsttype)((srctype*)(src))[_i];        \
  } while (0)

struct MatPartitioning_ParHIP
{
  // HACK: this must be at the same byte offset in the struct as the same field in the parmetis
  // struct
  PetscInt cuts;
  PetscInt seed;
  PetscInt mode;
...
PetscErrorCode MatPartitioningSetFromOptions_ParHIP(MatPartitioning part,
                                                    PetscOptionItems PetscOptionsObject)
{
  ...
  PetscCall(PetscOptionsReal("-parhip_imbalance", "partition imbalance", NULL, ctx->imbalance,
                             &ctx->imbalance, NULL));
  PetscCall(PetscOptionsInt("-parhip_seed", "random seed", NULL, ctx->seed, &ctx->seed, NULL));
  PetscCall(PetscOptionsBool("-parhip_suppress_output", "suppress parhip logging", NULL,
                             ctx->suppress_output, &ctx->suppress_output, NULL));
  PetscCall(PetscOptionsInt("-parhip_mode", "parhip configuration mode", NULL, ctx->mode,
                            &ctx->mode, NULL));
...
PetscErrorCode MatPartitioningApply_ParHIP(MatPartitioning part, IS* partition)
{
  ...
  PetscCheck(sizeof(PetscInt) <= sizeof(idxtype), PETSC_COMM_SELF, PETSC_ERR_PLIB,
             "ParHIP: sizeof(PetscInt) == %lu must be at most sizeof(idxtype) == %lu",
             sizeof(PetscInt), sizeof(idxtype));
  PetscCall(PetscMalloc6(n + 1, &xadj, m, &adjncy, n, &parts, n, &p_parts, n, &vtxwgt,
                         comm_size + 1, &vtxdist));
  ...
  ParHIPPartitionKWay(vtxdist, xadj, adjncy, vtxwgt, adjcwgt, &nparts, &imbalance, suppress, seed,
                      mode, &edgecut, parts, &comm);
