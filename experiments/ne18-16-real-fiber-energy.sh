...
# use the spack env's python when it provides the pipeline's packages (no manual env
# activation needed; the system python's matplotlib is incompatible with plot.py on the
# pde cluster), fall back to the system python (e.g. locally, where the view is bare)
if [ -z "${PYTHON:-}" ]; then
