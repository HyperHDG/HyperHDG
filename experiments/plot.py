...
def render_tikz(idx, name0, series, refs, args):
    ...
    L = [
        ...
        "% override \\figurewidth/\\figureheight (size of the axis frame, excluding labels)",
        "% and \\plotdatadir (default empty, set with trailing /) as needed",
        # trailing % guards the newlines: when \input mid-paragraph, bare line ends
        # would become stray interword spaces before the picture
        "\\providecommand{\\figurewidth}{.9\\linewidth}%",
        "\\providecommand{\\figureheight}{.9\\linewidth}%",
        "\\providecommand{\\plotdatadir}{}%",
        "% matplotlib tab10 colors, so tikz and matplotlib output match",
        "\\providecolor{mplC0}{HTML}{1F77B4}%",
        "\\providecolor{mplC1}{HTML}{FF7F0E}%",
        "\\providecolor{mplC2}{HTML}{2CA02C}%",
        "\\providecolor{mplC3}{HTML}{D62728}%",
        "\\providecolor{mplC4}{HTML}{9467BD}%",
        "\\providecolor{mplC5}{HTML}{8C564B}%",
        "% baseline: picture baseline at the lower-left frame corner, so side-by-side",
        "% plots align on the frame (use with \\begin{minipage}[b] or plain \\hfill rows)",
        "\\begin{tikzpicture}[baseline]",
    ]

    # scale only axis: width/height fix the frame itself, not frame plus labels,
    # so all plots get identical frames regardless of their tick labels
    axis_opts = [
        "scale only axis",
        "width=\\figurewidth, height=\\figureheight",
        "cycle list={{mplC0},{mplC1},{mplC2},{mplC3},{mplC4},{mplC5}}",
    ]
    if args.log:
