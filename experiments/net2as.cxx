...
PetscErrorCode MatCOO_Push(MatCOO* coo, PetscInt row, PetscInt col, PetscReal val)
{
  ...
  PetscFunctionBegin;
  PetscAssert(nnz < coo->cap, PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE,
              "MatCOO_Push out of range");
  coo->rows[nnz] = row;
  coo->cols[nnz] = col;
  coo->vals[nnz] = val;
  coo->nnz++;
...
PetscErrorCode MatCOO_Push_s(MatCOO* coo, PetscInt row, PetscInt col, PetscReal val)
{
  ...
  PetscFunctionBegin;
  PetscCheck(nnz < coo->cap, PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE,
             "MatCOO_Push out of range");
  coo->rows[nnz] = row;
  coo->cols[nnz] = col;
  coo->vals[nnz] = val;
...
PetscErrorCode net2as_alloc_ds(PC_Net2AS* data, PetscInt sz)
{
  ...
  PetscFunctionBegin;
  PetscCall(PetscMalloc6(min_sz, &data->ksp, min_sz, &data->is, min_sz, &data->sol, min_sz,
                         &data->sc, min_sz, &data->sd_gids, min_sz, &data->local_is));
  data->sz = min_sz;
  PetscFunctionReturn(0);
}
...
PetscErrorCode PCSetFromOptions_Net2AS(PC pc, PetscOptionItems PetscOptionsObject)
{
  ...
  PetscCall(
    PetscOptionsBoundedInt("-net2as_p", "number of subdomains per axis", NULL, p, &p, &set, p_lb));
  if (set)
    data->p[0] = data->p[1] = p;
  PetscCall(PetscOptionsBoundedInt("-net2as_px", "number of subdomains", NULL, data->p[0],
                                   &data->p[0], &set, p_lb));
  PetscCall(PetscOptionsBoundedInt("-net2as_py", "number of subdomains", NULL, data->p[1],
                                   &data->p[1], &set, p_lb));
  PetscCall(PetscOptionsBool("-net2as_print_local", "wether to print the local sizes", NULL,
                             data->print_local, &data->print_local, &set));
  PetscCall(PetscOptionsReal("-net2as_eps", "filter tolerance", NULL, data->eps, &data->eps, &set));
  PetscCall(PetscOptionsInt(
    "-net2as_mult",
    "upper bound on the (pointwise) multiplicity of the cover formed by the subdomains", NULL,
    data->mult_bound, &data->mult_bound, &set));
  PetscCall(PetscOptionsInt("-net2as_delta", "overlap parameters in number of hops", NULL,
                            data->delta, &data->delta, &set));
  PetscCall(PetscOptionsInt("-net2as_pux_dim", "number of dimension to extend the pu by", NULL,
                            data->pux_dim, &data->pux_dim, &set));
  PetscCall(PetscOptionsString("-net2as_cb_type", "subdomain partition type", NULL, data->cb_type,
                               data->cb_type, sizeof(data->cb_type), &set));
  PetscCall(PetscOptionsString("-net2as_load_type", "subdomain load balancing type", NULL,
                               data->load_type, data->load_type, sizeof(data->load_type), &set));
  PetscOptionsHeadEnd();
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static inline PetscBool net2as_is_dirichlet(PetscInt type, PetscBool wave)
{
  if (type == 0)
    return PETSC_FALSE;
  if (wave && (type & (1u << 6)))
    return PETSC_FALSE;  // static-only, free in wave
  return PETSC_TRUE;
}
                                 ...
                                 const PetscInt* edges_global)
{
  ...
  for (PetscInt i = 0; i < n_owned_nodes * sdim; i++)
    arr[i] = coords[i];
...
PetscErrorCode net2as_setup_ds(PC pc,
                               MPI_Comm comm,
                               const char* prefix,
                               KSP* ksp,
                               Mat* mat,
...
PetscErrorCode net2as_loadbalance_greedy(MPI_Comm comm,
                                         PetscInt* weights,
                                         PetscInt* assignments,
                                         PetscInt count,
                                         PetscReal* bal)
{
  int size;
...
PetscErrorCode net2as_loadbalance_round_robin(MPI_Comm comm,
                                              PetscInt* weights,
                                              PetscInt* assignments,
                                              PetscInt count,
                                              PetscReal* bal)
...
PetscErrorCode net2as_distribute_subdomains(MPI_Comm comm, PC_Net2AS* data, MatCOO* cb, MatCOO* sd)
{
  ...
  PetscCallMPI(MPI_Comm_size(comm, &size));
  PetscCall(PetscCalloc7(p, &sd2lcounts, p, &sd2gcounts, p, &sd2rank, size, &rank2scount, size,
                         &rank2rcount, cb->nnz, &coo2rank, 4 * size, &reqs));
  ...
  if (rank == 0)
  {
    if (strcmp(data->load_type, "rr") == 0)
      ...
      PetscCheck(false, PETSC_COMM_WORLD, PETSC_ERR_ARG_UNKNOWN_TYPE,
                 "unexptected load balancing type '%s', expected one of 'rr', 'gr'",
                 data->load_type);
  }
  ...
  // now rank2rcount specifies from which remote rank the local rank will receive how many vertex,
  // sd_id pairs
  ...
  {
    PetscCallMPI(
      MPI_Irecv(sd->cols + off, rank2rcount[r], MPIU_INT, r, tag_vid, comm, &reqs[2 * r]));
    PetscCallMPI(
      MPI_Irecv(sd->rows + off, rank2rcount[r], MPIU_INT, r, tag_sid, comm, &reqs[2 * r + 1]));
    off += rank2rcount[r];
  }
  sd->nnz = off;
  ...
  {
    PetscCallMPI(
      MPI_Isend(tmp_rows + off, rank2scount[r], MPIU_INT, r, tag_vid, comm, &reqs[2 * (size + r)]));
    PetscCallMPI(MPI_Isend(tmp_cols + off, rank2scount[r], MPIU_INT, r, tag_sid, comm,
                           &reqs[2 * (size + r) + 1]));
    off += rank2scount[r];
  }
  ...
  while (start < sd->nnz)
  {
    ...
    // PetscCall(ISCreateBlock(PETSC_COMM_SELF, data->bs, end-start, global_vertex_ids,
    // PETSC_COPY_VALUES, &data->is[off]));
    PetscCall(ISCreateGeneral(PETSC_COMM_SELF, end - start, global_vertex_ids, PETSC_COPY_VALUES,
                              &data->is[off]));
    start = end;
    off++;
  }
  PetscCheck(off == sd_count, PETSC_COMM_SELF, PETSC_ERR_PLIB,
             "detected '%d' subdomains, expected '%d'", off, sd_count);
  ...
  PetscCall(PetscFree7(sd2lcounts, sd2gcounts, sd2rank, rank2scount, rank2rcount, coo2rank, reqs));
  PetscFunctionReturn(0);
}
...
PetscErrorCode net2as_make_is_local(PC_Net2AS* data, MatCOO* sd)
{
  ...
  for (PetscInt i = 0; i < data->sz; i++)
  {
    ...
    PetscCheck(n_global == n_local, PETSC_COMM_WORLD, PETSC_ERR_PLIB,
               "rank_is local to global mapping inds dropped: expected %" PetscInt_FMT
               ", got %" PetscInt_FMT,
               n_global, n_local);
  }
...
PetscErrorCode net2as_cb_pu(PC_Net2AS* data, MatCOO* coo, MatCOO* sd)
{
  ...
  // longer sums to 1 across the rank boundary. Reduce to the global per-node subdomain count:
  // scatter the local counts onto a node-level global vector (ADD), then broadcast the totals back.
  {
    Vec gcount, lcount;
    VecScatter sc_count;
    PetscScalar* la;
    ...
    PetscCall(VecGetArray(lcount, &la));
    for (PetscInt i = 0; i < nloc; i++)
      counts[i] = (PetscInt)(PetscRealPart(la[i]) + 0.5);
    PetscCall(VecRestoreArray(lcount, &la));
  ...
  for (PetscInt s = 0; s < data->sz; s++)
  {
    ...
    PetscReal centroid[3] = {0}, min_sd[3] = {PETSC_MAX_REAL, PETSC_MAX_REAL, PETSC_MAX_REAL},
              max_sd[3] = {PETSC_MIN_REAL, PETSC_MIN_REAL, PETSC_MIN_REAL};
    ...
    PetscCheck(sz == sz2, PETSC_COMM_SELF, PETSC_ERR_PLIB,
               "local and global is size should be the same, %" PetscInt_FMT " != %" PetscInt_FMT,
               sz, sz2);
    for (PetscInt i = 0; i < sz; i++)
    {
    ...
    for (PetscInt i = 0; i < sz; i++)
    {
      ...
      for (PetscInt j = 0; j < data->pux_dim; j++)
        PetscCall(
          MatCOO_Push(coo, inds[i], cb_idx + j + 1,
                      (points[3 * li + j] - centroid[j]) / (max_sd[j] - min_sd[j]) / counts[li]));
    }
...
PetscErrorCode PCSetup_Net2AS(PC pc)
{
  ...
  PetscCheck(size % 3 == 0, comm, PETSC_ERR_ARG_SIZ,
             "size of points = %" PetscInt_FMT " must be divisible by 3 (x0,y0,z0,x1,y1,z1,...)",
             size);
  ...
  if (strcmp(data->cb_type, "q1") == 0)
    ...
    PetscCheck(false, PETSC_COMM_WORLD, PETSC_ERR_ARG_UNKNOWN_TYPE,
               "unsupported type '%s', muse be one of 'q1', 'pu'", data->cb_type);
  ...
  PetscCall(
    MatSetSizes(coarse_basis, cb_local_rows, PETSC_DECIDE, size, n_cols * (data->pux_dim + 1)));
  ...
  PetscCall(
    PetscPrintf(PETSC_COMM_WORLD, "    nz_mat: %" PetscInt_FMT "\n", (PetscInt)mat_info.nz_used));
  PetscCall(MatGetSize(data->cmat, &m, &n));
  PetscCall(PetscPrintf(PETSC_COMM_WORLD, "  coarse:\n"));
  PetscCall(PetscPrintf(PETSC_COMM_WORLD, "    size: %" PetscInt_FMT "\n", m));
  PetscCall(
    net2as_setup_ds(pc, PETSC_COMM_WORLD, "coarse_", &data->cksp, &data->cmat, &data->csol, &info));
  ...
  if (data->print_local)
    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "  local:\n"));
  for (PetscInt i = 0; i < data->sz; i++)
  {
    PetscCall(
      net2as_setup_ds(pc, PETSC_COMM_SELF, "", &data->ksp[i], &data->mat[i], &data->sol[i], &info));
    PetscCall(
      VecScatterCreate(data->rank_sol, data->local_is[i], data->sol[i], NULL, &data->sc[i]));
    t_loc += info.time;
    if (data->print_local)
    {
      PetscCall(
        PetscSynchronizedPrintf(PETSC_COMM_WORLD, "    - size: %" PetscInt_FMT "\n", info.size));
      ...
      PetscCall(
        PetscSynchronizedPrintf(PETSC_COMM_WORLD, "      factor_type: %s\n", info.factor_type));
    }
  }
...
PetscErrorCode PCApply_Net2AS(PC pc, Vec x, Vec y)
{
  ...
  // local
  // NOTE: separate loops are required as otherwise the reverse scatter would modify the other rank
  // entries
  for (PetscInt i = 0; i < data->sz; i++)
  {
    PetscCall(
      VecScatterBegin(data->sc[i], data->rank_sol, data->sol[i], INSERT_VALUES, SCATTER_FORWARD));
    PetscCall(
      VecScatterEnd(data->sc[i], data->rank_sol, data->sol[i], INSERT_VALUES, SCATTER_FORWARD));
  }
  ...
  {
    PetscCall(
      VecScatterBegin(data->sc[i], data->sol[i], data->rank_sol, ADD_VALUES, SCATTER_REVERSE));
    PetscCall(
      VecScatterEnd(data->sc[i], data->sol[i], data->rank_sol, ADD_VALUES, SCATTER_REVERSE));
  }
