...
# net3 CLAMP rung: the 20mm x 20mm center patch of net3 (--clamp-xy 0.5 of 40mm,
# ~15M edges, ~61M trace dofs) -- sized to pde12 (1TB): est. ~700 GB peak, ~30%
# headroom (measured net2 scaling model, chat 2026-07-12); full net3 stays ~2.4 TB.
# Same material as net2 (c_shear = 1.722e9 um/s); tap scaled with the domain:
# std_x = 250 um, std_t = std_x/c ~ 1.5e-7, tap = 5 std_t, dt = 7.5e-8 = std_t/2.
# center->rim = 10 mm -> arrival at 7.5e-7 + 5.8e-6; T = 6.3e-6 (NT=84): front at
# ~9.6 mm, just before the clamped rim.  Per-cell energies stay ON (~46 GB at
# stride 2).  net2as_p 16: 256 subdomains >= 128 ranks (pde12) and H = 1.25mm ~ 5 R0.
: ${NT:=84}
: ${DT:=7.5e-8}
...
NET="-pc_type net2as -net2as_p 16 -net2as_cb_type q1 -net2as_cb_trim -net2as_pc_factor_mat_solver_type cholmod -net2as_coarse_pc_type lu"
...
$PYTHON experiments/gortz_constants.py $DOM --mu --cells 4 8 16 \
  --csv $OUT/gortz-net3clamp2raw.csv | tee $OUT/gortz-net3clamp2raw.txt
