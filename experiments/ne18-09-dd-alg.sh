...
mpirun -n 8 $BUILD/network -mat_cache $MAT -test constant -domain $DOMAIN \
       $NET -net2as_p 4 -net2as_print_local -mem_max -ksp_monitor_yaml -net2as_cb_type q1 \
       | yq -o json -I0 >> $LOG
mpirun -n 8 $BUILD/network -mat_cache $MAT -test constant -domain $DOMAIN \
       $NET -net2as_p 4 -net2as_print_local -mem_max -ksp_monitor_yaml -net2as_cb_type pu \
       | yq -o json -I0 >> $LOG
mpirun -n 8 $BUILD/network -mat_cache $MAT -test constant -domain $DOMAIN \
       $NET -net2as_p 4 -net2as_print_local -mem_max -ksp_monitor_yaml -net2as_cb_type pu -net2as_nocoarse \
       | yq -o json -I0 >> $LOG
jq '{iterations, type: .net2as.cb_type}' output/ne18-09-dd-alg/log.json
#done
