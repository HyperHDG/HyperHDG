...
OMP_NUM_THREADS=2 $BUILD/network -domain $DOMAIN  -comp 2 -strain .15 $NET \
               -trace_view hdf5:$TRACE -plot $STATIC
OMP_NUM_THREADS=2 $BUILD/timowave -domain $DOMAIN -comp 2 -strain .15 -nt 100 -T 1e-3 $NET \
                -static $TRACE -plot $WAVE -deg 3 -net2as_wave -log_view :$PERF:ascii_flamegraph \
                -print_timestep | tee $LOG
experiments/netvis.py $WAVE --view pside --frames 0,.5e-3,1e-3 --frame-colors tan,purple,blue \
