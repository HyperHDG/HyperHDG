...
def read_morgan(path):
  ...
  return nodes, edges, edgeProps, info

def data_cleanup(nodes, edges, edgeProps):
...
def compute_types(nodes, edges, dims):
  sides = {
    "xmin": nodes[:,0] - mins[0] < args.t * dims[0],
  ...
  return types_points, types_faces


def drop_floating_and_small_components(nodes, edges, edgeProps, types_points, types_faces):
  n_nodes = nodes.shape[0]
  n_edgeProps = edgeProps.shape[0]
  ...
  return nodes, edges, edgeProps, types_points, types_faces

def write_h5(nodes, edges, edgeProps, info, types_points, types_faces):
...
if __name__ == "__main__":
  ...
  nodes, edges, edgeProps, info = read_morgan(args.i)
  data_cleanup(nodes, edges, edgeProps)
  mins = nodes.min(axis=0)
  maxs = nodes.max(axis=0)
  dims = maxs - mins
  info["size"] = dims
  tprint("info", info)
  types_points, types_faces = compute_types(nodes, edges, dims)
  drop_floating_and_small_components(nodes, edges, edgeProps, types_points, types_faces)
  write_h5(nodes, edges, edgeProps, info, types_points, types_faces)
