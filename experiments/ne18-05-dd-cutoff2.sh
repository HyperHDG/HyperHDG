...
for clip in 2 10 30; do
  python experiments/make_geo2.py -i $INPUT --clamp-xy $CUT -o $OUT/domain.$clip.geo.h5 --dirichlet xmax=68 xmin=63 --prop-cutoff $clip
  for p in 1 2 4 8 16; do
    mpirun -n 8 $BUILD/network -domain $OUT/domain.$clip.geo.h5 -comp 2 -strain .15 $NET -net2as_p $p \
                   | yq -o=json -I0 ".clip = $clip" >> $LOG
  done
done
