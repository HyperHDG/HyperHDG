...
void print_bdr_values(const LVecT& lambda, const CVecT& coeff, const Bdr_type bdr)
{
  ...
  std::cout << "u:\t" << coeff[0] + f * r3 * coeff[si] << "\t"
            << coeff[sa] + f * r3 * coeff[sa + si] << "\n";
  ...
  if (bdr == RIGHT)
    std::cout << "v^:\t" << lambda[4] << "\t" << lambda[5] << "\n";
  std::cout << "q:\t" << coeff[4] + f * r3 * coeff[4 + si] << "\t"
            << coeff[4 + sa] + f * r3 * coeff[4 + sa + si] << "\n";
  std::cout << "p:\t" << coeff[2 * 4] + f * r3 * coeff[2 * 4 + si] << "\t"
            << coeff[2 * 4 + sa] + f * r3 * coeff[2 * 4 + sa + si] << "\n";
  std::cout << "v:\t" << coeff[4 * 4] + f * r3 * coeff[4 * 4 + si] << "\t"
            << coeff[4 * 4 + sa] + f * r3 * coeff[4 * 4 + sa + si] << "\n";
  std::cout << "z:\t" << coeff[5 * 4] + f * r3 * coeff[5 * 4 + si] << "\t"
            << coeff[5 * 4 + sa] + f * r3 * coeff[5 * 4 + sa + si] << "\n";
  return;
}
...
{
  std::cout << "u:\t" << coeff[0 * 4 + 0] << "\t" << coeff[0 * 4 + 1] << "\t" << coeff[0 * 4 + 2]
            << "\t" << coeff[0 * 4 + 3] << "\n";
  std::cout << "q:\t" << coeff[1 * 4 + 0] << "\t" << coeff[1 * 4 + 1] << "\t" << coeff[1 * 4 + 2]
            << "\t" << coeff[1 * 4 + 3] << "\n";
  std::cout << "p:\t" << coeff[2 * 4 + 0] << "\t" << coeff[2 * 4 + 1] << "\t" << coeff[2 * 4 + 2]
            << "\t" << coeff[2 * 4 + 3] << "\n";
  std::cout << "s:\t" << coeff[3 * 4 + 0] << "\t" << coeff[3 * 4 + 1] << "\t" << coeff[3 * 4 + 2]
            << "\t" << coeff[3 * 4 + 3] << "\n";
  std::cout << "v:\t" << coeff[4 * 4 + 0] << "\t" << coeff[4 * 4 + 1] << "\t" << coeff[4 * 4 + 2]
            << "\t" << coeff[4 * 4 + 3] << "\n";
  std::cout << "z:\t" << coeff[5 * 4 + 0] << "\t" << coeff[5 * 4 + 1] << "\t" << coeff[5 * 4 + 2]
            << "\t" << coeff[5 * 4 + 3] << "\n";
  std::cout << "r:\t" << coeff[6 * 4 + 0] << "\t" << coeff[6 * 4 + 1] << "\t" << coeff[6 * 4 + 2]
            << "\t" << coeff[6 * 4 + 3] << "\n";
  return;
}
...
int main()
{
  ...
  HDGHyperGraph<lst::n_glob_dofs_per_node(), Topology::File<2, 2>, Geometry::File<2, 2>,
                NodeDescriptor::File<2, 2>, lst::data_type>
  ...
  std::for_each(hg.begin(), hg.end(),
                [&](auto he)
                {
                  ...
                  */
                  ...
                  for (unsigned int n = 0; n < 4; ++n)
                  {
                    std::for_each(lambda_n[n].begin(), lambda_n[n].end(),
                                  [](auto i) { std::cout << i << "\t"; });
                    std::cout << "\n";
                  ...
                  for (unsigned int n = 0; n < 4; ++n)
                  {
                    std::for_each(res_flux[n].begin(), res_flux[n].end(),
                                  [](auto i) { std::cout << i << "\t"; });
                    std::cout << "\n";
                  }
                  for (unsigned int n = 0; n < 4; ++n)
                  ...
                  int i = 0;
                  // std::for_each(res.begin(), res.end(), [&i](double e) {std::cout << i++ << "\t"
                  // << e << "\n";});
                  ...
                  f0 = 1000. * f0;
                  i = 0;
                  // std::for_each(f0.begin(), f0.end(), [&i](double e) {std::cout << i++ << "\t" <<
                  // e << "\n";});

                  res = res - f0;
                  i = 0;
                  std::for_each(res.begin(), res.end(),
                                [&i](double e) { std::cout << i++ << "\t" << e << "\n"; });
                  ...
                  res_flux = ls.coupling_coeff_directional_derivative(lambda_n, coeff, coeff_dir,
                                                                      res_flux, he, 1.);
                  for (int n = 0; n < 4; ++n)
                    for (int i = 0; i < 6; ++i)
                      std::cout << n << " " << i << " " << res_flux[n][i] - out[n][i] << "\n";
