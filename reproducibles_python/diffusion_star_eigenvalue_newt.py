...
def eigenvalue_newt(poly_degree, dimension, level, iteration, initial="default", debug_mode=False):
  ...
  f = open("output/diffusion_star_convergence_eigenvalue_newton.txt", "a")
  f.write("Polynomial degree = " + str(poly_degree) + ". Dimension = " + str(dimension) \
          + ". Level = " + str(level) + ". Iteration = " + str(iteration) + ". Eigenvalue = " + str(eig_val) + ".\n")
  f.close()
  
  HDG_wrapper.plot_option( "fileName" , "diff_star_eig_newt_dim-" + str(dimension) + "-lvl-" + str(level) + "-" + str(iteration) )
    ...
    for poly_degree in range(1, 3):
      ...
      for level in range(0, 4):
        ...
        for iteration in range(0, 3):
          try:
            eigenvalue_newt(poly_degree, dimension, level, iteration, "default", debug_mode)
