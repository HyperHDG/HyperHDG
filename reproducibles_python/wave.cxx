...
PetscErrorCode PetscHDGCreate(PetscInt space_dim,
                              PetscInt poly_deg,
                              PetscInt nx,
                              PetscReal tau,
                              PetscReal theta,
                              ...
                              HDGBase** hdg)
{
  ...
  {
    case 31:
      *hdg = new HDGWrapper(HDGWave<3, 1>(nx, {tau, theta, dt}));
      return 0;
    case 32:
      *hdg = new HDGWrapper(HDGWave<3, 2>(nx, {tau, theta, dt}));
      return 0;
    case 61:
      *hdg = new HDGWrapper(HDGWave<6, 1>(nx, {tau, theta, dt}));
      return 0;
    case 62:
      *hdg = new HDGWrapper(HDGWave<6, 2>(nx, {tau, theta, dt}));
      return 0;
    default:
...
int main(int argc, char** argv)
{
  ...
  PetscCall(PetscOptionsReal("-tau",
                             ...
                             NULL, tau, &tau, &is_set));
  PetscCall(
    PetscOptionsInt("-nx", "number of elements divide the domain into", NULL, nx, &nx, &is_set));
  PetscCall(PetscOptionsInt("-nt", "number of timesteps", NULL, nt, &nt, &is_set));
  PetscCall(PetscOptionsReal("-T", "end time", NULL, T, &T, &is_set));
  PetscCall(PetscOptionsString("-o", "output filename", NULL, output_filename, output_filename,
                               PATH_MAX, &is_set));
  PetscCall(PetscOptionsString("-od", "output directory", NULL, output_directory, output_directory,
                               PATH_MAX, &is_set));
  PetscCall(PetscOptionsBool("-plot", "plot solution", NULL, plot, &plot, &is_set));
  PetscCall(PetscOptionsString("-plot_scale", "subdomain scale factor for plotting", NULL,
                               plot_scale, plot_scale, PATH_MAX, &is_set));
  PetscOptionsEnd();
  ...
  PetscCall(MatSetPreallocationCOO(mat, mat_coo.value_vec.size(), (PetscInt*)mat_coo.row_vec.data(),
