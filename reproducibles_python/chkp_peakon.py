...
def diffusion_test(poly_degree, iteration, debug_mode=False):
  ...
  for time_step in range(time_steps):
    ...
    if round(time_steps * time / goal_time) % (time_steps // 30) == 0:
    ...
    u_error = errors[0]
    q_error = errors[1]
    if round(time_steps * (time - start_time) / (goal_time - start_time) ) % (time_steps // 30) == 0 or time == start_time + delta_time:
