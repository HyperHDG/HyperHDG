...
def eigenvalue_approx_MA(poly_degree, dimension, iteration, level, debug_mode=False):
  ...
  f = open("output/diffusion_L_eigenvalue_approx.txt", "a")
  ...
  HDG_wrapper.plot_option( "fileName" , "diff_L_eig_apprx_pd-" + str(poly_degree) + "-lvl-" + str(level) + "-" + str(iteration));
  ...
  for poly_degree in range(1,3):
    ...
    for level in range(0,3):
      ...
      for iteration in range(2,4):
        eigenvalue_approx_MA(poly_degree, 2, iteration, level, debug_mode)
