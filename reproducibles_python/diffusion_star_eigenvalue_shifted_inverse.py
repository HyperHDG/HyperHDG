...
def eigenvalue_approx_SI(poly_degree, dimension, iteration, level, debug_mode=False):
  ...
  const.node_descriptor = "File<" + str(hy_dim) + "," + str(dimension) + ",std::vector,Point<" + str(
    dimension) + ",double> >"
  ...
  f = open("output/diffusion_star_eigenvalue_shifted_inverse.txt", "a")
  ...
  HDG_wrapper.plot_option( "fileName" , "diff_star_shifi-dim-" + str(dimension) + "-lvl-" + str(level) + "-" + str(iteration) )
...
def main(debug_mode):
  for poly_degree in range(1, 3):
    ...
    for dimension in range(2, 3):
      print("\n Dimension is set to be " + str(poly_degree) + "\n\n")
      for level in range(0, 4):
        print("Level is ", level, "\n")
        for iteration in range(2,5):
          eigenvalue_approx_SI(poly_degree, dimension, iteration, level, debug_mode)
