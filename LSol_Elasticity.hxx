...
class ElasticitySolver_RegularQuad
{
  ...
  public:
    ...
    std::array< std::array<double, compute_n_dofs_per_node(hyEdge_dim, max_poly_degree)> , 2 * hyEdge_dim >
      preprocess_data(std::array< std::array<double, space_dim * compute_n_dofs_per_node(hyEdge_dim, max_poly_degree)> , 2*hyEdge_dim >& hyperedge_dofs, typename Geometry::UnitCube<hyEdge_dim, space_dim>::value_type& geometry ) const;
    std::array< std::array<double, space_dim * compute_n_dofs_per_node(hyEdge_dim, max_poly_degree)> , 2 * hyEdge_dim >
      postprocess_data(std::array< std::array<double, compute_n_dofs_per_node(hyEdge_dim, max_poly_degree)> , 2*hyEdge_dim >& hyperedge_dofs, typename Geometry::UnitCube<hyEdge_dim, space_dim>::value_type& geometry ) const;
    ...
    static constexpr unsigned int solution_dimension_hyperedge() { return hyEdge_dim; }
